...
package com.flint.tools.flintc.processing;
...
public class JavacProcessingEnvironment implements ProcessingEnvironment, Closeable {
    ...
    private void discoverAndRunProcs(Set<TypeElement> annotationsPresent,
        ...
        while(unmatchedAnnotations.size() > 0 && psi.hasNext() ) {
            ...
            for (Entry<String, TypeElement> entry: unmatchedAnnotations.entrySet()) {
    ...
    private boolean isPkgInfo(JavaFileObject fo, Kind kind) {
        return fo.isNameCompatible("package-info", kind);
    }

    private boolean isPkgInfo(ClassSymbol sym) {
        return isPkgInfo(sym.classfile, Kind.CLASS) && (sym.packge().package_info == sym);
    }

    private boolean isModuleInfo(JavaFileObject fo, Kind kind) {
