...
import static com.flint.tools.flintc.parser.TokenKind.ASSERT;
import static com.flint.tools.flintc.parser.TokenKind.CASE;
import static com.flint.tools.flintc.parser.TokenKind.CATCH;
import static com.flint.tools.flintc.parser.TokenKind.EQ;
import static com.flint.tools.flintc.parser.TokenKind.GT;
import static com.flint.tools.flintc.parser.TokenKind.IMPORT;
import static com.flint.tools.flintc.parser.TokenKind.LT;
import static com.flint.tools.flintc.parser.TokenKind.*;
import static com.flint.tools.flintc.parser.Tokens.DUMMY;
...
//    private Source source;
    ...
    private JCList<JCAnnotation> typeAnnotationsPushedBack = JCList.nil();
...
//        this.source = fac.source;
        ...
        this.allowEffectivelyFinalVariablesInTWR =
                true; //source.allowEffectivelyFinalVariablesInTryWithResources();
        this.allowDiamond = true; //source.allowDiamond();
        this.allowMulticatch = true; //source.allowMulticatch();
        this.allowStringFolding = fac.options.getBoolean("allowStringFolding", true);
        this.allowLambda = true; //source.allowLambda();
        this.allowMethodReferences = true; //source.allowMethodReferences();
        this.allowDefaultMethods = true; //source.allowDefaultMethods();
        this.allowStaticInterfaceMethods = true; //source.allowStaticInterfaceMethods();
        this.allowIntersectionTypesInCast = true; //source.allowIntersectionTypesInCast();
        this.allowTypeAnnotations = true; //source.allowTypeAnnotations();
        this.allowModules = true; //source.allowModules();
        this.allowAnnotationsAfterTypeParams = true; //source.allowAnnotationsAfterTypeParams();
        this.allowUnderscoreIdentifier = true; //source.allowUnderscoreIdentifier();
        this.allowPrivateInterfaceMethods = true; //source.allowPrivateInterfaceMethods();
    ...
    protected JCErroneous syntaxError(int pos, String key, TokenKind... args) {
        return syntaxError(pos, JCList.nil(), key, args);
    }

    protected JCErroneous syntaxError(int pos, JCList<JCTree> errs, String key, TokenKind... args) {
        setErrorEndPos(pos);
...
/* ---------- parsing -------------- */
    ...
    public JCExpression qualident(boolean allowAnnos) {
        ...
        while (token.kind == DOT) {
            int pos = token.pos;
            nextToken();
            JCList<JCAnnotation> tyannos = null;
            if (allowAnnos) {
    ...
    public JCExpression parseType() {
        JCList<JCAnnotation> annotations = typeAnnotationsOpt();
        return parseType(annotations);
    }

    public JCExpression parseType(JCList<JCAnnotation> annotations) {
    ...
    //where
        ...
        protected JCExpression foldStrings(JCExpression tree) {
            ...
            if (needsFolding) {
                JCList<JCExpression> ops = opStack.toList();
    ...
    protected JCExpression term3() {
        int pos = token.pos;
        JCExpression t;
        JCList<JCExpression> typeArgs = typeArgumentsOpt(EXPR);
        switch (token.kind) {
        case QUES:
            ...
            if (typeArgs == null && (mode & EXPR) != 0) {
                ...
                switch (pres) {
                    case CAST:
                       ...
                       mode = TYPE;
                       int pos1 = pos;
                       JCList<JCExpression> targets = JCList.of(t = term3());
        ...
        case MONKEYS_AT:
            ...
            JCList<JCAnnotation> typeAnnos = typeAnnotationsOpt();
        ...
        case UNDERSCORE: case IDENTIFIER: case ASSERT: case ENUM:
            ...
            if ((mode & EXPR) != 0 && peekToken(ARROW)) {
                ...
                loop: while (true) {
                    pos = token.pos;
                    final JCList<JCAnnotation> annos = typeAnnotationsOpt();
    ...
    JCExpression term3Rest(JCExpression t, JCList<JCExpression> typeArgs) {
        if (typeArgs != null) illegal();
        while (true) {
            int pos1 = token.pos;
            final JCList<JCAnnotation> annos = typeAnnotationsOpt();
    ...
    JCExpression lambdaExpressionOrStatement(boolean hasParens, boolean explicitParams, int pos) {
        JCList<JCVariableDecl> params = explicitParams ?
    ...
    JCExpression lambdaExpressionOrStatementRest(JCList<JCVariableDecl> args, int pos) {
        checkLambda();
        accept(ARROW);
    ...
    JCExpression lambdaStatement(JCList<JCVariableDecl> args, int pos, int pos2) {
        JCBlock block = block(pos2, 0);
        return toP(F.at(pos).Lambda(args, block));
    }

    JCExpression lambdaExpression(JCList<JCVariableDecl> args, int pos) {
     ...
     */
    JCExpression superSuffix(JCList<JCExpression> typeArgs, JCExpression t) {
        nextToken();
     ...
     */
    JCExpression argumentsOpt(JCList<JCExpression> typeArgs, JCExpression t) {
    ...
    JCMethodInvocation arguments(JCList<JCExpression> typeArgs, JCExpression t) {
        int pos = token.pos;
        JCList<JCExpression> args = arguments();
    ...
    JCList<JCExpression> typeArgumentsOpt() {
        return typeArgumentsOpt(TYPE);
    }

    JCList<JCExpression> typeArgumentsOpt(int useMode) {
        if (token.kind == LT) {
            if ((mode & useMode) == 0 ||
                (mode & NOPARAMS) != 0) {
                illegal();
     ...
     *  }
     */
    JCList<JCExpression> typeArguments(boolean diamondAllowed) {
        if (token.kind == LT) {
            nextToken();
            ...
            return JCList.of(syntaxError(token.pos, "expected", LT));
        }
    }
    ...
    JCExpression typeArgument() {
        ...
        if (token.kind == EXTENDS) {
            ...
            JCErroneous err = F.at(pos).Erroneous(JCList.<JCTree>of(wc, id));
    ...
    JCTypeApply typeArguments(JCExpression t, boolean diamondAllowed) {
        int pos = token.pos;
        JCList<JCExpression> args = typeArguments(diamondAllowed);
    ...
    private JCExpression bracketsOpt(JCExpression t,
            JCList<JCAnnotation> annotations) {
        JCList<JCAnnotation> nextLevelAnnotations = typeAnnotationsOpt();
     ...
     */
    private JCExpression bracketsOpt(JCExpression t) {
        return bracketsOpt(t, JCList.nil());
    }
    ...
    JCExpression bracketsSuffix(JCExpression t) {
        if ((mode & EXPR) != 0 && token.kind == DOT) {
            ...
            if (token.pos == endPosTable.errorEndPos) {
                ...
                t = F.at(pos).Erroneous(JCList.<JCTree>of(toP(F.at(pos).Select(t, name))));
            } else {
                JCTreeTag tag = t.getTag();
            ...
            if (token.kind != TokenKind.COLCOL) {
                mode = TYPE;
            }
        } else if (token.kind != TokenKind.COLCOL) {
    ...
    JCExpression memberReferenceSuffix(int pos1, JCExpression t) {
        ...
        mode = EXPR;
        JCList<JCExpression> typeArgs = null;
        if (token.kind == LT) {
     ...
     */
    JCExpression creator(int newpos, JCList<JCExpression> typeArgs) {
        JCList<JCAnnotation> newAnnotations = typeAnnotationsOpt();
        ...
        while (token.kind == TokenKind.DOT) {
            ...
            int pos = token.pos;
            nextToken();
            JCList<JCAnnotation> tyannos = typeAnnotationsOpt();
            t = toP(F.at(pos).Select(t, ident()));
        ...
        mode = oldmode;
        if (token.kind == TokenKind.LBRACKET || token.kind == TokenKind.MONKEYS_AT) {
            ...
            if (diamondFound) {
                ...
                return toP(F.at(newpos).Erroneous(JCList.of(e)));
            }
            ...
            if (newClass.def != null) {
                ...
                if (newAnnotations.nonEmpty()) {
                    ...
                    // com.flint.tools.flintc.code.TypeAnnotations.TypeAnnotationPositions.visitNewClass(JCNewClass)
            ...
            t = toP(F.at(newpos).NewClass(null, typeArgs, t, JCList.nil(), null));
            return toP(F.at(newpos).Erroneous(JCList.<JCTree>of(t)));
        }
    }

    /** InnerCreator = [Annotations] Ident [TypeArguments] ClassCreatorRest
     */
    JCExpression innerCreator(int newpos, JCList<JCExpression> typeArgs, JCExpression encl) {
        JCList<JCAnnotation> newAnnotations = typeAnnotationsOpt();
    ...
    JCExpression arrayCreatorRest(int newpos, JCExpression elemtype) {
        JCList<JCAnnotation> annos = typeAnnotationsOpt();
        ...
        if (token.kind == RBRACKET) {
            ...
            if (token.kind == LBRACE) {
                ...
                JCExpression t = toP(F.at(newpos).NewArray(elemtype, JCList.nil(), null));
                return syntaxError(token.pos, JCList.of(t), "array.dimension.missing");
            }
        } else {
            ListBuffer<JCExpression> dims = new ListBuffer<>();

            // maintain array dimension type annotations
            ListBuffer<JCList<JCAnnotation>> dimAnnotations = new ListBuffer<>();
            ...
            while (token.kind == LBRACKET
                ...
                JCList<JCAnnotation> maybeDimAnnos = typeAnnotationsOpt();
                int pos = token.pos;
                nextToken();
                if (token.kind == RBRACKET) {
                                  ...
                                  JCExpression t)
    {
        ...
        if (token.kind == LBRACE) {
            int pos = token.pos;
            JCList<JCTree> defs = classOrInterfaceBody(names.empty, false);
    ...
    JCExpression arrayInitializer(int newpos, JCExpression t) {
        ...
        accept(RBRACE);
        return toP(F.at(newpos).NewArray(t, JCList.nil(), elems.toList()));
    }
    ...
    JCBlock block(int pos, long flags) {
        accept(LBRACE);
        JCList<JCStatement> stats = blockStatements();
     ...
     */
    @SuppressWarnings("fallthrough")
    JCList<JCStatement> blockStatements() {
        //todo: skip to anchor on error(?)
        int lastErrPos = -1;
        ListBuffer<JCStatement> stats = new ListBuffer<>();
        while (true) {
            JCList<JCStatement> stat = blockStatement();
            if (stat.isEmpty()) {
                return stats.toList();
            } else {
                // error recovery
    ...
    JCStatement parseStatementAsBlock() {
        int pos = token.pos;
        JCList<JCStatement> stats = blockStatement();
        if (stats.isEmpty()) {
            ...
            if (error != null) {
                error(first, error);
                JCList<JCBlock> blist = JCList.of(F.at(first.pos).Block(0, stats));
     ...
     */
    JCList<JCStatement> blockStatement() {
        //todo: skip to anchor on error(?)
        int pos = token.pos;
        switch (token.kind) {
        ...
        case ASSERT:
            return JCList.of(parseSimpleStatement());
        case MONKEYS_AT:
        case FINAL: {
            ...
            if (token.kind == INTERFACE ||
                token.kind == CLASS ||
                token.kind == ENUM) {
                return JCList.of(classOrInterfaceOrEnumDeclaration(mods));
            } else {
        ...
        case CLASS:
            ...
            return JCList.of(classOrInterfaceOrEnumDeclaration(modifiersOpt()));
        case ENUM:
        ...
        default:
            ...
            if (token.kind == COLON && t.hasTag(IDENT)) {
                ...
                return JCList.of(F.at(pos).Labelled(prevToken.name(), stat));
    ...
    public JCStatement parseSimpleStatement() {
        ...
        case FOR: {
            nextToken();
            accept(LPAREN);
            JCList<JCStatement> inits = token.kind == SEMI ? JCList.nil() : forInit();
            if (inits.length() == 1 &&
                ...
                accept(SEMI);
                JCList<JCExpressionStatement> steps = token.kind == RPAREN ? JCList.nil() : forUpdate();
                accept(RPAREN);
                JCStatement body = parseStatementAsBlock();
        ...
        case TRY: {
            nextToken();
            JCList<JCTree> resources = JCList.nil();
            if (token.kind == LPAREN) {
        ...
        case SWITCH: {
            ...
            accept(LBRACE);
            JCList<JCCase> cases = switchBlockStatementGroups();
    ...
    private JCStatement doRecover(int startPos, ErrorRecoveryAction action, String key) {
        ...
        S.errPos(errPos);
        return toP(F.Exec(syntaxError(startPos, JCList.of(stm), key)));
    }
    ...
    protected JCCatch catchClause() {
        ...
        JCList<JCExpression> catchTypes = catchTypes();
    ...
    JCList<JCExpression> catchTypes() {
     ...
     */
    JCList<JCCase> switchBlockStatementGroups() {
    ...
    JCList<JCStatement> forInit() {
        ...
        if (token.kind == FINAL || token.kind == MONKEYS_AT) {
            ...
            if ((lastmode & TYPE) != 0 && LAX_IDENTIFIER.accepts(token.kind)) {
                ...
                return JCList.of((JCStatement)F.at(pos).VarDef(null, null, t, null));
            } else {
     ...
     */
    JCList<JCExpressionStatement> forUpdate() {
     ...
     */
    protected JCList<JCAnnotation> annotationsOpt(JCTreeTag kind) {
        if (token.kind != MONKEYS_AT) return JCList.nil(); // optimization
        ...
        lastmode = mode;
        mode = prevmode;
        JCList<JCAnnotation> annotations = buf.toList();

        return annotations;
    }

    JCList<JCAnnotation> typeAnnotationsOpt() {
        JCList<JCAnnotation> annotations = annotationsOpt(JCTreeTag.TYPE_ANNOTATION);
        return annotations;
    }
    ...
    loop:
        while (true) {
            ...
            if (flag == Flags.ANNOTATION) {
                if (token.kind != INTERFACE) {
                    JCAnnotation ann = annotation(lastPos, JCTreeTag.ANNOTATION);
     ...
     * @param kind Whether to parse an ANNOTATION or TYPE_ANNOTATION
     */
    JCAnnotation annotation(int pos, JCTreeTag kind) {
        // accept(AT); // AT consumed by caller
        if (kind == JCTreeTag.TYPE_ANNOTATION) {
            checkTypeAnnotations();
        }
        JCTree ident = qualident(false);
        JCList<JCExpression> fieldValues = annotationFieldValuesOpt();
        JCAnnotation ann;
        if (kind == JCTreeTag.ANNOTATION) {
            ann = F.at(pos).Annotation(ident, fieldValues);
        } else if (kind == JCTreeTag.TYPE_ANNOTATION) {
    ...
    JCList<JCExpression> annotationFieldValuesOpt() {
        return (token.kind == LPAREN) ? annotationFieldValues() : JCList.nil();
    }

    /** AnnotationFieldValues   = "(" [ AnnotationFieldValue { "," AnnotationFieldValue } ] ")" */
    JCList<JCExpression> annotationFieldValues() {
        accept(LPAREN);
    ...
    JCExpression annotationValue() {
        ...
        case MONKEYS_AT:
            pos = token.pos;
            nextToken();
            return annotation(pos, JCTreeTag.ANNOTATION);
        case LBRACE:
            pos = token.pos;
            accept(LBRACE);
            ListBuffer<JCExpression> buf = new ListBuffer<>();
            ...
            accept(RBRACE);
            return toP(F.at(pos).NewArray(null, JCList.nil(), buf.toList()));
        default:
            mode = EXPR;
            return term1();
        }
    ...
    public <T extends ListBuffer<? super JCVariableDecl>> T variableDeclarators(JCModifiers mods,
                                                                         ...
                                                                         T vdefs)
    {
        return  variableDeclaratorsRest(token.pos, mods, type, ident(), false, vdefs);
    }
    ...
    protected <T extends ListBuffer<? super JCVariableDecl>> T variableDeclaratorsRest(int pos,
                                                                     ...
                                                                     T vdefs)
    {
        vdefs.append(variableDeclaratorRest(pos, mods, type, name, reqInit));//));
        while (token.kind == COMMA) {
            // All but last of multiple declarators subsume a comma
            storeEnd((JCTree)vdefs.last(), token.endPos);
            nextToken();
            vdefs.append(variableDeclarator(mods, type, reqInit));
        }
        return vdefs;
    }
     ...
     */
    JCVariableDecl variableDeclarator(JCModifiers mods, JCExpression type, boolean reqInit) {
        return variableDeclaratorRest(token.pos, mods, type, ident(), reqInit);
    }
    ...
    JCVariableDecl variableDeclaratorId(JCModifiers mods, JCExpression type, boolean lambdaParameter) {
        ...
        if (lambdaParameter && token.kind == UNDERSCORE) {
            ...
            if (allowThisIdent && !lambdaParameter) {
                ...
                if (pn.hasTag(JCTreeTag.IDENT) && ((JCIdent)pn).name != names._this) {
    ...
    protected JCTree resource() {
        ...
        if (token.kind == FINAL || token.kind == MONKEYS_AT) {
            ...
            JCExpression t = parseType();
            return variableDeclaratorRest(token.pos, mods, t, ident(), true);
        }
        JCExpression t = term(EXPR | TYPE);
        if ((lastmode & TYPE) != 0 && LAX_IDENTIFIER.accepts(token.kind)) {
    ...
    public JCTree.JCCompilationUnit parseCompilationUnit() {
        ...
        if (token.kind == PACKAGE) {
            ...
            JCList<JCAnnotation> annotations = JCList.nil();
            seenPackage = true;
            if (mods != null) {
            ...
            if (checkForImports && mods == null && token.kind == IMPORT) {
                ...
                if (firstTypeDecl && token.kind == IDENTIFIER) {
                    ...
                    if (token.kind == IDENTIFIER && token.name() == names.module) {
                        ...
                        defs.append(moduleDecl(mods, kind));
                        consumedToplevelDoc = true;
                        break;
    ...
    JCModuleDecl moduleDecl(JCModifiers mods, ModuleKind kind) {
        int pos = token.pos;
        if (!allowModules) {
        ...
        JCList<JCDirective> directives = null;
    ...
    JCList<JCDirective> moduleDirectiveList() {
        ...
        while (token.kind == IDENTIFIER) {
            ...
            loop:
                ...
                JCList<JCExpression> moduleNames = null;
                ...
                if (token.kind == IDENTIFIER && token.name() == names.with) {
                    nextToken();
                    JCList<JCExpression> implNames = qualidentList(false);
                    accept(SEMI);
    ...
    JCTree typeDeclaration(JCModifiers mods) {
        ...
        if (mods == null && token.kind == SEMI) {
            ...
            return classOrInterfaceOrEnumDeclaration(modifiersOpt(mods));
        }
    }
    ...
    protected JCStatement classOrInterfaceOrEnumDeclaration(JCModifiers mods) {
        if (token.kind == CLASS) {
            ...
            return enumDeclaration(mods);
        } else {
            int pos = token.pos;
            JCList<JCTree> errs;
            if (LAX_IDENTIFIER.accepts(token.kind)) {
                errs = JCList.of(mods, toP(F.at(pos).Ident(ident())));
                setErrorEndPos(token.pos);
            } else {
                errs = JCList.of(mods);
            }
     ...
     */
    protected JCClassDecl classDeclaration(JCModifiers mods) {
        int pos = token.pos;
        accept(CLASS);
        Name name = ident();

        JCList<JCTypeParameter> typarams = typeParametersOpt();
        ...
        JCList<JCExpression> implementing = JCList.nil();
        if (token.kind == IMPLEMENTS) {
            nextToken();
            implementing = typeList();
        }
        JCList<JCTree> defs = classOrInterfaceBody(name, false);
     ...
     */
    protected JCClassDecl interfaceDeclaration(JCModifiers mods) {
        int pos = token.pos;
        accept(INTERFACE);
        Name name = ident();
        ...
        JCList<JCExpression> extending = JCList.nil();
        if (token.kind == EXTENDS) {
            nextToken();
            extending = typeList();
        }
        JCList<JCTree> defs = classOrInterfaceBody(name, true);
        JCClassDecl result = toP(F.at(pos).ClassDef(
     ...
     */
    protected JCClassDecl enumDeclaration(JCModifiers mods) {
        int pos = token.pos;
        accept(ENUM);
        Name name = ident();
        ...
        JCList<JCTree> defs = enumBody(name);
        mods.flags |= Flags.ENUM;
        JCClassDecl result = toP(F.at(pos).
            ClassDef(mods, name, JCList.nil(),
     ...
     */
    JCList<JCTree> enumBody(Name enumName) {
        accept(LBRACE);
        ListBuffer<JCTree> defs = new ListBuffer<>();
        if (token.kind == COMMA) {
            nextToken();
    ...
    JCTree enumeratorDeclaration(Name enumName) {
        ...
        int pos = token.pos;
        JCList<JCAnnotation> annotations = annotationsOpt(JCTreeTag.ANNOTATION);
        JCModifiers mods = F.at(annotations.isEmpty() ? Position.NOPOS : pos).Modifiers(flags, annotations);
        JCList<JCExpression> typeArgs = typeArgumentsOpt();
        int identPos = token.pos;
        Name name = ident();
        int createPos = token.pos;
        JCList<JCExpression> args = (token.kind == LPAREN)
            ? arguments() : JCList.nil();
        JCClassDecl body = null;
        if (token.kind == LBRACE) {
     ...
     */
    JCList<JCTree> classOrInterfaceBody(Name className, boolean isInterface) {
        accept(LBRACE);
        if (token.pos <= endPosTable.errorEndPos) {
            // error recovery
     ...
     *
     */
    protected JCList<JCTree> classOrInterfaceBodyDeclaration(Name className, boolean isInterface) {
        if (token.kind == SEMI) {
            nextToken();
            return JCList.nil();
        } else {
            ...
            if (token.kind == CLASS ||
                ...
                return JCList.of(block(pos, mods.flags));
            } else {
                pos = token.pos;
                JCList<JCTypeParameter> typarams = typeParametersOpt();
                ...
                JCList<JCAnnotation> annosAfterParams = annotationsOpt(JCTreeTag.ANNOTATION);
                ...
                if (token.kind == LPAREN && !isInterface && type.hasTag(IDENT)) {
                    ...
                    return JCList.of(methodDeclaratorRest(
                    ...
                    if (token.kind == LPAREN) {
                        ...
                        JCList<JCTree> defs =
                            variableDeclaratorsRest(pos, mods, type, name, isInterface,
                        ...
                        pos = token.pos;
                        JCList<JCTree> err = isVoid
                            ? JCList.of(toP(F.at(pos).MethodDef(mods, name, type, typarams,
                                JCList.nil(), JCList.nil(), null, null)))
                            : null;
                        return JCList.of(syntaxError(token.pos, err, "expected", LPAREN));
                    }
                }
            }
        }
    ...
    protected JCTree methodDeclaratorRest(int pos,
        ...
        try {
            ...
            JCList<JCVariableDecl> params = formalParameters();
            if (!isVoid) type = bracketsOpt(type);
            JCList<JCExpression> thrown = JCList.nil();
     ...
     */
    JCList<JCExpression> qualidentList(boolean allowAnnos) {
        ListBuffer<JCExpression> ts = new ListBuffer<>();

        JCList<JCAnnotation> typeAnnos = allowAnnos ? typeAnnotationsOpt() : JCList.nil();
        ...
        while (token.kind == COMMA) {
            ...
            typeAnnos = allowAnnos ? typeAnnotationsOpt() : JCList.nil();
     ...
     *  }
     */
    protected JCList<JCTypeParameter> typeParametersOpt() {
        if (token.kind == LT) {
     ...
     */
    JCList<JCVariableDecl> formalParameters() {
        return formalParameters(false);
    }
    JCList<JCVariableDecl> formalParameters(boolean lambdaParameters) {
    ...
    JCList<JCVariableDecl> implicitParameters(boolean hasParens) {
        if (hasParens) {
            accept(LPAREN);
        }
        ListBuffer<JCVariableDecl> params = new ListBuffer<>();
    ...
    private JCExpression insertAnnotationsToMostInner(
            JCExpression type, JCList<JCAnnotation> annos,
    ...
    protected JCVariableDecl formalParameter(boolean lambdaParameter) {
        ...
        if (token.kind == ELLIPSIS) {
            JCList<JCAnnotation> varargsAnnos = typeAnnotationsPushedBack;
            typeAnnotationsPushedBack = JCList.nil();
...
/* ---------- auxiliary methods -------------- */
    ...
    protected JCExpression checkExprStat(JCExpression t) {
        if (!TreeInfo.isExpressionStatement(t)) {
            JCExpression ret = F.at(t.pos).Erroneous(JCList.<JCTree>of(t));
            error(ret, "not.stmt");
            return ret;
        } else {
            return t;
     ...
     */
    static JCTreeTag optag(TokenKind token) {
        switch (token) {
        case BARBAR:
            return OR;
        case AMPAMP:
     ...
     *  No_TAG if token is not a binary operator.
     */
    static JCTreeTag unoptag(TokenKind token) {
        switch (token) {
        case PLUS:
            return POS;
        case SUB:
