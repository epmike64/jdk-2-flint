...
import com.flint.tools.flintc.code.Symbol.*;
import com.flint.tools.flintc.code.Type.*;
import com.flint.tools.flintc.code.Types.UniqueType;
import com.flint.tools.flintc.jvm.Pool.DynamicMethod;
import com.flint.tools.flintc.jvm.Pool.Method;
import com.flint.tools.flintc.jvm.Pool.MethodHandle;
import com.flint.tools.flintc.jvm.Pool.Variable;
 ...
 ******************************************************************/
    ...
    private void writeParamAnnotations(JCList<VarSymbol> params,
 ...
 **********************************************************************/
     ...
     */
    int writeJavaAnnotations(JCList<Attribute.Compound> attrs) {
    ...
    int writeTypeAnnotations(JCList<Attribute.TypeCompound> typeAnnos, boolean inCode) {
 ...
 **********************************************************************/
    ...
    int writeModuleAttribute(ClassSymbol c) {
        ...
        JCList<ExportsDirective> exports = m.exports;
        ...
        JCList<OpensDirective> opens = m.opens;
        ...
        JCList<UsesDirective> uses = m.uses;
...
//          log.errWriter.println("enter inner " + c);//DEBUG
    ...
    void writeInnerClasses() {
        ...
        for (JCList<ClassSymbol> l = innerClassesQueue.toList();
             l.nonEmpty();
             l = l.tail) {
    ...
    void writeMethod(MethodSymbol m) {
        ...
        JCList<Type> thrown = m.erasure(types).getThrownTypes();
        if (thrown.nonEmpty()) {
    ...
    void writeCode(Code code) {
        ...
        for (JCList<char[]> l = code.catchInfo.toList();
             l.nonEmpty();
             l = l.tail) {
        ...
        if (code.lineInfo.nonEmpty()) {
            ...
            for (JCList<char[]> l = code.lineInfo.reverse();
                 l.nonEmpty();
                 l = l.tail)
                for (int i = 0; i < l.head.length; i++)
                    databuf.appendChar(l.head[i]);
    ...
    void writeFields(Scope s) {
        ...
        JCList<VarSymbol> vars = JCList.nil();
    ...
    void writeMethods(Scope s) {
        JCList<MethodSymbol> methods = JCList.nil();
        for (Symbol sym : s.getSymbols(NON_RECURSIVE)) {
    ...
    public void writeClassFile(OutputStream out, ClassSymbol c)
        ...
        JCList<Type> interfaces = types.interfaces(c.type);
        JCList<Type> typarams = c.type.getTypeArguments();
        ...
        for (JCList<Type> l = interfaces; l.nonEmpty(); l = l.tail)
            databuf.appendChar(pool.put(l.head.tsym));
        int fieldsCount = 0;
        int methodsCount = 0;
        ...
        for (JCList<Type> l = interfaces; !sigReq && l.nonEmpty(); l = l.tail)
