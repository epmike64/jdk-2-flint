...
import com.flint.tools.flintc.model.FilteredMemberList;
...
import static com.flint.tools.flintc.jvm.CRTFlags.*;
...
public class Gen extends JCTree.Visitor {
    ...
    private void checkDimension(DiagnosticPosition pos, Type t) {
        ...
        case METHOD:
            ...
            for (JCList<Type> args = t.getParameterTypes(); args.nonEmpty(); args = args.tail)
 ...
 *************************************************************************/
     ...
     */
    JCList<JCTree> normalizeDefs(JCList<JCTree> defs, ClassSymbol c) {
        ...
        for (JCList<JCTree> l = defs; l.nonEmpty(); l = l.tail) {
            JCTree def = l.head;
            switch (def.getTag()) {
            case BLOCK:
        ...
        if (initCode.length() != 0) {
            JCList<JCStatement> inits = initCode.toList();
            initTAs.addAll(c.getInitTypeAttributes());
            JCList<TypeCompound> initTAlist = initTAs.toList();
        ...
        if (clinitCode.length() != 0) {
            MethodSymbol clinit = new MethodSymbol(
                ...
                names.clinit,
                new MethodType(
                    JCList.nil(), syms.voidType,
                    JCList.nil(), syms.methodClass),
                c);
            c.members().enter(clinit);
            JCList<JCStatement> clinitStats = clinitCode.toList();
    ...
    private JCList<TypeCompound> getAndRemoveNonFieldTAs(VarSymbol sym) {
        JCList<TypeCompound> tas = sym.getRawTypeAttributes();
     ...
     */
    void normalizeMethod(JCMethodDecl md, JCList<JCStatement> initCode, JCList<TypeCompound> initTAs) {
        if (md.name == names.init && TreeInfo.isInitialConstructor(md)) {
            // We are seeing a constructor that does not call another
            // constructor of the same class.
            JCList<JCStatement> stats = md.body.stats;
 ...
 *************************************************************************/
    ...
    public void genStat(JCTree tree, Env<GenContext> env, int crtFlags) {
        ...
        genStat(tree, env);
        if (tree.hasTag(JCTreeTag.BLOCK)) crtFlags |= CRT_BLOCK;
     ...
     *                   indicating type of the entry.
     */
    public void genStats(JCList<JCStatement> trees, Env<GenContext> env, int crtFlags) {
        if (!genCrt) {
            genStats(trees, env);
            return;
        }
     ...
     */
    public void genStats(JCList<? extends JCTree> trees, Env<GenContext> env) {
     ...
     */
    public void genArgs(JCList<JCExpression> trees, JCList<Type> pts) {
...
//where
    ...
    public void visitDoLoop(JCDoWhileLoop tree) {
        genLoop(tree, tree.body, tree.cond, JCList.nil(), false);
    }

    public void visitWhileLoop(JCWhileLoop tree) {
        genLoop(tree, tree.body, tree.cond, JCList.nil(), true);
    }
    ...
    public void visitSwitch(JCSwitch tree) {
        ...
        JCList<JCCase> cases = tree.cases;
        if (cases.isEmpty()) {
...
//where
    ...
    public void visitSynchronized(JCSynchronized tree) {
        ...
        genTry(tree.body, JCList.nil(), syncEnv);
        code.endScopes(limit);
    }
    ...
    //where
         ...
         */
        void genTry(JCTree body, JCList<JCCatch> catchers, Env<GenContext> env) {
            int limit = code.nextreg;
            int startpc = code.curCP();
            ...
            JCList<Integer> gaps = env.info.gaps.toList();
            code.statBegin(TreeInfo.endPos(body));
            genFinalizer(env);
            code.statBegin(TreeInfo.endPos(env.tree));
            Chain exitChain = code.branch(goto_);
            endFinalizerGap(env);
            if (startpc != endpc) for (JCList<JCCatch> l = catchers; l.nonEmpty(); l = l.tail) {
        ...
        void genCatch(JCCatch tree,
                      ...
                      JCList<Integer> gaps) {
            if (startpc != endpc) {
                JCList<Pair<JCList<TypeCompound>, JCExpression>> catchTypeExprs
                        = catchTypesWithAnnotations(tree);
                while (gaps.nonEmpty()) {
                    for (Pair<JCList<TypeCompound>, JCExpression> subCatch1 : catchTypeExprs) {
        ...
        // where
        JCList<Pair<JCList<TypeCompound>, JCExpression>> catchTypesWithAnnotations(JCCatch tree) {
            return TreeInfo.isMultiCatch(tree) ?
                    ...
                    JCList.of(new Pair<>(tree.param.sym.getRawTypeAttributes(), tree.param.vartype));
        }
        // where
        JCList<Pair<JCList<TypeCompound>, JCExpression>> catchTypesWithAnnotationsFromMulticatch(JCTypeUnion tree, JCList<TypeCompound> first) {
            JCList<JCExpression> alts = tree.alternatives;
            JCList<Pair<JCList<TypeCompound>, JCExpression>> res = JCList.of(new Pair<>(first, alts.head));
            alts = alts.tail;
            ...
            while(alts != null && alts.head != null) {
                ...
                if (alt instanceof JCAnnotatedType) {
                    ...
                    res = res.prepend(new Pair<>(JCList.nil(), alt));
                }
                alts = alts.tail;
            }
            return res.reverse();
 ...
 *************************************************************************/
    ...
    public void visitNewArray(JCNewArray tree) {
        ...
        if (tree.elems != null) {
            ...
            int i = 0;
            for (JCList<JCExpression> l = tree.elems; l.nonEmpty(); l = l.tail) {
                arr.duplicate();
                loadIntConst(i);
                i++;
...
//where
    ...
    private void genNullCheck(DiagnosticPosition pos) {
        if (allowBetterNullChecks) {
            callMethod(pos, syms.objectsType, names.requireNonNull,
                    JCList.of(syms.objectType), true);
        } else {
    ...
    private void generateReferencesToPrunedTree(ClassSymbol classSymbol, Pool pool) {
        JCList<JCTree> prunedInfo = lower.prunedTree.get(classSymbol);
        if (prunedInfo != null) {
 ...
 *************************************************************************/
    ...
    public boolean genClass(Env<AttrContext> env, JCClassDecl cdef) {
        try {
            ...
            for (JCList<JCTree> l = cdef.defs; l.nonEmpty(); l = l.tail) {
            ...
            cdef.defs = JCList.nil(); // discard trees
            return nerrs == 0;
        } finally {
