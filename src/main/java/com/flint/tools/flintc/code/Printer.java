...
public abstract class Printer implements Type.Visitor<String, Locale>, Symbol.Visitor<String, Locale> {
    ...
    JCList<Type> seenCaptured = JCList.nil();
     ...
     * @return localized string representation
     */
    public String visitTypes(JCList<Type> ts, Locale locale) {
     ...
     * @return localized string representation
     */
    public String visitSymbols(JCList<Symbol> ts, Locale locale) {
        ListBuffer<String> sbuf = new ListBuffer<>();
        for (Symbol t : ts) {
            sbuf.append(visit(t, locale));
        }
    ...
    private String printAnnotations(Type t, boolean prefix) {
        ...
        JCList<Attribute.TypeCompound> annos = t.getAnnotationMirrors();
        if (!annos.isEmpty()) {
            if (prefix) sb.append(' ');
            sb.append(annos);
            sb.append(' ');
    ...
    protected String className(ClassType t, boolean longform, Locale locale) {
        ...
        if (sym.name.length() == 0 && (sym.flags() & COMPOUND) != 0) {
            ...
            for (JCList<Type> is = t.interfaces_field; is.nonEmpty(); is = is.tail) {
                s.append('&');
     ...
     * @return localized string representation
     */
    protected String printMethodArgs(JCList<Type> args, boolean varArgs, Locale locale) {
        if (!varArgs) {
