...
import com.flint.tools.flintc.code.Type.BottomType;
import com.flint.tools.flintc.code.Type.ClassType;
import com.flint.tools.flintc.code.Type.ErrorType;
import com.flint.tools.flintc.code.Type.JCPrimitiveType;
import com.flint.tools.flintc.code.Type.JCVoidType;
import com.flint.tools.flintc.code.Type.MethodType;
import com.flint.tools.flintc.code.Type.UnknownType;
...
public class Symtab {
    ...
    private Type enterSyntheticAnnotation(String name) {
        ...
        type.typarams_field = JCList.nil();
        type.allparams_field = JCList.nil();
        type.supertype_field = annotationType;
        type.interfaces_field = JCList.nil();
        return type;
    }
    ...
    protected Symtab(Context context) throws CompletionFailure {
        ...
        unnamedModule = new ModuleSymbol(names.empty, null) {
                {
                    directives = JCList.nil();
                    exports = JCList.nil();
                    provides = JCList.nil();
                    uses = JCList.nil();
                    ModuleSymbol java_base = enterModule(names.java_base);
                    com.flint.tools.flintc.code.Directive.RequiresDirective d =
                            new com.flint.tools.flintc.code.Directive.RequiresDirective(java_base,
                                    EnumSet.of(com.flint.tools.flintc.code.Directive.RequiresFlag.MANDATED));
                    requires = JCList.of(d);
                }
                @Override
        ...
        enumFinalFinalize =
            new MethodSymbol(PROTECTED|FINAL|HYPOTHETICAL,
                             names.finalize,
                             new MethodType(JCList.nil(), voidType,
                                            JCList.nil(), methodClass),
                             enumSym);
        ...
        autoCloseableClose = new MethodSymbol(PUBLIC,
                             names.close,
                             new MethodType(JCList.nil(), voidType,
                                            JCList.of(exceptionType), methodClass),
        ...
        arrayClassType.interfaces_field = JCList.of(cloneableType, serializableType);
        ...
        arrayCloneMethod = new MethodSymbol(
            PUBLIC,
            names.clone,
            new MethodType(JCList.nil(), objectType,
                           JCList.nil(), methodClass),
            arrayClass);
    ...
    public JCList<ModuleSymbol> listPackageModules(Name packageName) {
        if (packageName.isEmpty())
            return JCList.nil();

        JCList<ModuleSymbol> result = JCList.nil();
        Map<ModuleSymbol,PackageSymbol> map = packages.get(packageName);
        if (map != null) {
            for (Map.Entry<ModuleSymbol, PackageSymbol> e: map.entrySet()) {
                if (!e.getValue().members().isEmpty()) {
