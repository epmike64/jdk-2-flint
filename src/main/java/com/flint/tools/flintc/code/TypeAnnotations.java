...
import com.flint.tools.flintc.code.Attribute.Array;
...
import com.flint.tools.flintc.code.Type.CapturedType;
import com.flint.tools.flintc.code.Type.ClassType;
import com.flint.tools.flintc.code.Type.ErrorType;
import com.flint.tools.flintc.code.Type.ForAll;
import com.flint.tools.flintc.code.Type.MethodType;
import com.flint.tools.flintc.code.Type.PackageType;
import com.flint.tools.flintc.code.Type.TypeVar;
import com.flint.tools.flintc.code.Type.UndetVar;
import com.flint.tools.flintc.code.Type.Visitor;
import com.flint.tools.flintc.code.Type.WildcardType;
import com.flint.tools.flintc.code.TypeAnnotationPosition.TypePathEntry;
import com.flint.tools.flintc.code.TypeAnnotationPosition.TypePathEntryKind;
...
public class TypeAnnotations {
    ...
    public JCList<Attribute> annotationTargets(TypeSymbol tsym) {
        ...
        JCList<Attribute> targets = ((Array)atValue).getValue();
    ...
    public AnnotationType annotationTargetType(Attribute.Compound a, Symbol s) {
        JCList<Attribute> targets = annotationTargets(a.type.tsym);
        return (targets == null) ?
    ...
    private class TypeAnnotationPositions extends TreeScanner {
         ...
         */
        private JCList<JCTree> frames = JCList.nil();
        ...
        {
            JCList<Attribute.Compound> allAnnotations = sym.getRawAttributes();
            ...
            JCList<TypeCompound> typeAnnotations = typeAnnos.toList();
            ...
            if (sym.getKind() == ElementKind.METHOD) {
                ...
                if (sym.getQualifiedName().equals(names._this)) {
                    ...
                    JCList<VarSymbol> params = ((MethodSymbol)sym.owner).params;
                    JCList<Type> oldArgs = methType.argtypes;
        ...
        // {@link com.flint.tools.flintc.parser.JavacParser.insertAnnotationsToMostInner(JCExpression, List<JCTypeAnnotation>, boolean)}
        ...
        private Type typeWithAnnotations(final JCTree typetree, final Type type,
                                         final JCList<TypeCompound> annotations,
                                         final JCList<TypeCompound> onlyTypeAnnotations,
                                         final TypeAnnotationPosition pos)
        {
            ...
            if (type.hasTag(TypeTag.TYPEVAR)) {
                ...
                // UnionClassType uct = (com.flint.tools.flintc.code.Type.UnionClassType)type;
         ...
         */
        private Type rewriteArrayType(ArrayType type, JCList<TypeCompound> annotations, TypeAnnotationPosition pos) {
            ArrayType tomodify = new ArrayType(type);
            ArrayType res = tomodify;

            JCList<TypePathEntry> loc = JCList.nil();
        ...
        private Type typeWithAnnotations(final Type type,
                                         final Type stopAt,
                                         final JCList<TypeCompound> annotations) {
            Visitor<Type, JCList<TypeCompound>> visitor =
                    new Type.Visitor<Type, JCList<TypeCompound>>() {
                @Override
                public Type visitClassType(ClassType t, JCList<TypeCompound> s) {
                ...
                @Override
                public Type visitWildcardType(WildcardType t, JCList<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public Type visitArrayType(ArrayType t, JCList<TypeCompound> s) {
                ...
                @Override
                public Type visitMethodType(MethodType t, JCList<TypeCompound> s) {
                    // Impossible?
                    return t;
                }

                @Override
                public Type visitPackageType(PackageType t, JCList<TypeCompound> s) {
                    // Impossible?
                    return t;
                }

                @Override
                public Type visitTypeVar(TypeVar t, JCList<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public Type visitModuleType(ModuleType t, JCList<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public Type visitCapturedType(CapturedType t, JCList<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public Type visitForAll(ForAll t, JCList<TypeCompound> s) {
                    // Impossible?
                    return t;
                }

                @Override
                public Type visitUndetVar(UndetVar t, JCList<TypeCompound> s) {
                    // Impossible?
                    return t;
                }

                @Override
                public Type visitErrorType(ErrorType t, JCList<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public Type visitType(Type t, JCList<TypeCompound> s) {
                    return t.annotatedType(s);
                }
            };
                         ...
                         ListBuffer<TypePathEntry> location)
        {
            ...
            // com.flint.tools.flintc.jvm.Gen.setTypeAnnotationPositions(int)
            ...
            switch (frame.getKind()) {
                ...
                case PARAMETERIZED_TYPE: {
                    JCList<JCTree> newPath = path.tail;
                ...
                case ARRAY_TYPE: {
                    ...
                    while (true) {
                        ...
                        if (npHead.hasTag(JCTree.JCTreeTag.TYPEARRAY)) {
                            newPath = newPath.tail;
                            location = location.prepend(TypePathEntry.ARRAY);
                        } else if (npHead.hasTag(JCTree.JCTreeTag.ANNOTATED_TYPE)) {
                            newPath = newPath.tail;
                        } else {
                            break;
                        }
                ...
                case TYPE_PARAMETER:
                    if (path.tail.tail.head.hasTag(JCTree.JCTreeTag.CLASSDEF)) {
                        ...
                        return TypeAnnotationPosition
                            .typeParameterBound(location.toList(),
                                                ...
                                                frame.pos);
                    } else if (path.tail.tail.head.hasTag(JCTree.JCTreeTag.METHODDEF)) {
                ...
                case INTERSECTION_TYPE: {
                    ...
                    final JCList<JCTree> newPath = path.tail;
                    return resolveFrame(newPath.head, newPath.tail.head,
                                        newPath, currentLambda,
        ...
        public void visitNewArray(JCNewArray tree) {
            ...
            depth = depth.append(TypePathEntry.ARRAY);
            while (elemType != null) {
                if (elemType.hasTag(JCTree.JCTreeTag.ANNOTATED_TYPE)) {
                    JCAnnotatedType at = (JCAnnotatedType)elemType;
                    final ListBuffer<TypePathEntry> locationbuf =
                        locateNestedTypes(elemType.type,
                                          new ListBuffer<TypePathEntry>());
                    final JCList<TypePathEntry> location =
                ...
                } else if (elemType.hasTag(JCTree.JCTreeTag.TYPEARRAY)) {
                    depth = depth.append(TypePathEntry.ARRAY);
                    elemType = ((JCArrayTypeTree)elemType).elemtype;
                } else if (elemType.hasTag(JCTree.JCTreeTag.SELECT)) {
        ...
        private void findTypeCompoundPosition(JCTree tree, JCTree frame, JCList<TypeCompound> annotations) {
            if (!annotations.isEmpty()) {
                final TypeAnnotationPosition p =
        ...
        private void findPosition(JCTree tree, JCTree frame, JCList<JCAnnotation> annotations) {
            if (!annotations.isEmpty())
            {
                final TypeAnnotationPosition p =
                    resolveFrame(tree, frame, frames, currentLambda, 0, new ListBuffer<>());
        ...
        private void setTypeAnnotationPos(JCList<JCAnnotation> annotations, TypeAnnotationPosition position)
        {
