...
import com.flint.tools.flintc.jvm.ClassWriter;
...
import com.flint.tools.flintc.code.Kinds.KindSelector;
import com.flint.tools.flintc.code.Scope.WriteableScope;
import com.flint.tools.flintc.main.Option.PkgInfo;
import com.flint.tools.flintc.util.*;
import com.flint.tools.flintc.util.JCDiagnostic.DiagnosticPosition;

import com.flint.tools.flintc.code.Symbol.*;
import com.flint.tools.flintc.code.Symbol.OperatorSymbol.AccessCode;
import com.flint.tools.flintc.tree.JCTree.*;
import com.flint.tools.flintc.code.Type.*;
...
import static com.flint.tools.flintc.tree.JCTree.JCOperatorExpression.OperandPos.LEFT;
 ...
 *************************************************************************/
     ...
     */
    public Map<ClassSymbol, JCList<JCTree>> prunedTree = new WeakHashMap<>();
     ...
     */
    Map<ClassSymbol,JCList<VarSymbol>> freevarCache;
    ...
    class FreeVarCollector extends BasicFreeVarCollector {
        ...
        private void addFreeVar(VarSymbol v) {
            for (JCList<VarSymbol> l = fvs; l.nonEmpty(); l = l.tail)
                if (l.head == v) return;
            fvs = fvs.prepend(v);
        }
        ...
        void addFreeVars(ClassSymbol c) {
            JCList<VarSymbol> fvs = freevarCache.get(c);
            if (fvs != null) {
                for (JCList<VarSymbol> l = fvs; l.nonEmpty(); l = l.tail) {
                    addFreeVar(l.head);
                }
     ...
     */
    JCList<VarSymbol> freevars(ClassSymbol c)  {
        JCList<VarSymbol> fvs = freevarCache.get(c);
        if (fvs != null) {
            return fvs;
        }
    ...
    class EnumMapping {
        ...
        void translate() {
            ...
            JCExpression mapVarInit = make
                .NewArray(make.Type(syms.intType), JCList.of(size), null)
            ...
            for (Map.Entry<VarSymbol,Integer> e : values.entrySet()) {
                ...
                JCStatement _try = make.Try(make.Block(0, JCList.of(exec)), catcher, null);
                stmts.append(_try);
            }
 ...
 *************************************************************************/
     ...
     */
    JCNewClass makeNewClass(Type ctype, JCList<JCExpression> args) {
        JCNewClass tree = make.NewClass(null,
            null, make.QualIdent(ctype.tsym), args, null);
        tree.constructor = rs.resolveConstructor(
            make_pos, attrEnv, ctype, TreeInfo.types(args), JCList.nil());
        tree.type = ctype;
        return tree;
    }
     ...
     */
    JCUnary makeUnary(JCTree.JCTreeTag optag, JCExpression arg) {
     ...
     */
    JCBinary makeBinary(JCTree.JCTreeTag optag, JCExpression lhs, JCExpression rhs) {
     ...
     *  @param rhs      The operator's right argument.
     */
    JCAssignOp makeAssignop(JCTree.JCTreeTag optag, JCTree lhs, JCTree rhs) {
    ...
    JCExpression makeString(JCExpression tree) {
        if (!tree.type.isPrimitiveOrVoid()) {
            ...
            Symbol valueOfSym = lookupMethod(tree.pos(),
                                             ...
                                             syms.stringType,
                                             JCList.of(tree.type));
            return make.App(make.QualIdent(valueOfSym), JCList.of(tree));
        }
    }
    ...
    JCClassDecl makeEmptyClass(long flags, ClassSymbol owner, Name flatname,
        ...
        ctype.interfaces_field = JCList.nil();
 ...
 *************************************************************************/
     ...
     */
    void checkConflicts(JCList<JCTree> translatedTrees) {
     ...
     */
    private MethodSymbol lookupMethod(DiagnosticPosition pos, Name name, Type qual, JCList<Type> args) {
        return rs.resolveInternalMethod(pos, attrEnv, qual, name, args, JCList.nil());
    }

    /** Look up a constructor.
     */
    private MethodSymbol lookupConstructor(DiagnosticPosition pos, Type qual, JCList<Type> args) {
    ...
    private void checkAccessConstructorTags() {
        for (JCList<ClassSymbol> l = accessConstrTags; l.nonEmpty(); l = l.tail) {
 ...
 *************************************************************************/
     ...
     */
    private JCList<ClassSymbol> accessConstrTags;
     ...
     */
    private OperatorSymbol binaryAccessOperator(int acode, JCTreeTag tag) {
     ...
     */
    private static JCTree.JCTreeTag treeTag(OperatorSymbol operator) {
    ...
    MethodSymbol accessSymbol(Symbol sym, JCTree tree, JCTree enclOp,
        ...
        JCList<Type> argtypes;      // The argument types of the access method.
        Type restype;             // The result type of the access method.
        JCList<Type> thrown;        // The thrown exceptions of the access method.
        switch (vsym.kind) {
        case VAR:
            ...
            if (acode >= AccessCode.FIRSTASGOP.code) {
                ...
                if (operator.opcode == string_add)
                    argtypes = JCList.of(syms.objectType);
                else
                    argtypes = operator.type.getParameterTypes().tail;
            } else if (acode == AccessCode.ASSIGN.code)
                argtypes = JCList.of(vsym.erasure(types));
            else
                argtypes = JCList.nil();
    ...
    private void addPrunedInfo(JCTree tree) {
        JCList<JCTree> infoList = prunedTree.get(currentClass);
        infoList = (infoList == null) ? JCList.of(tree) : infoList.prepend(tree);
        ...
        case MTH: case VAR:
            if (sym.owner.kind == TYP) {
                ...
                if (accReq || baseReq) {
                    ...
                    if (accReq) {
                        JCList<JCExpression> args = JCList.nil();
                        if ((sym.flags() & STATIC) == 0) {
    ...
    Symbol accessConstructor(DiagnosticPosition pos, Symbol constr) {
        if (needsPrivateAccess(constr)) {
            ...
            if (aconstr == null) {
                JCList<Type> argtypes = constr.type.getParameterTypes();
...
//      System.err.println("access " + vsym + " with " + accessor);//DEBUG
        ...
        JCList<JCExpression> args;    // Any additional arguments to be passed along.
        if ((sym.flags() & STATIC) != 0) {
            ref = make.Ident(sym);
        ...
        if (sym.kind == VAR) {
            ...
            default:
                expr = make.Assignop(
                    treeTag(binaryAccessOperator(acode1, JCTree.JCTreeTag.NO_TAG)), ref, args.head);
                ((JCAssignOp) expr).operator = binaryAccessOperator(acode1, JCTree.JCTreeTag.NO_TAG);
            }
        ...
        // are accessible.
        for (JCList<JCVariableDecl> l = md.params; l.nonEmpty(); l = l.tail)
            l.head.vartype = access(l.head.vartype);
        md.restype = access(md.restype);
        for (JCList<JCExpression> l = md.thrown; l.nonEmpty(); l = l.tail)
 ...
 *************************************************************************/
     ...
     *  parameters.
     */
    JCList<VarSymbol> outerThisStack;
     ...
     */
    JCList<JCVariableDecl> freevarDefs(int pos, JCList<VarSymbol> freevars, Symbol owner) {
    ...
    JCList<JCVariableDecl> freevarDefs(int pos, JCList<VarSymbol> freevars, Symbol owner,
        ...
        JCList<JCVariableDecl> defs = JCList.nil();
        for (JCList<VarSymbol> l = freevars; l.nonEmpty(); l = l.tail) {
            VarSymbol v = l.head;
     ...
     */
    JCList<JCExpression> loadFreevars(DiagnosticPosition pos, JCList<VarSymbol> freevars) {
        JCList<JCExpression> args = JCList.nil();
        for (JCList<VarSymbol> l = freevars; l.nonEmpty(); l = l.tail)
...
//where
    ...
    private JCBlock makeTwrBlock(JCList<JCTree> resources, JCBlock block,
        ...
        JCBlock catchBlock = make.Block(0L, JCList.of(assign, rethrowStat));
    ...
    private JCBlock makeTwrFinallyClause(Symbol primaryException, JCExpression resource,
        ...
        if (closeResource == null && shouldUseCloseResourceMethod()) {
            closeResource = new MethodSymbol(
                ...
                dollarCloseResource,
                new MethodType(
                    JCList.of(syms.throwableType, syms.autoCloseableType),
                    syms.voidType,
                    JCList.nil(),
                    syms.methodClass),
                currentClass);
            ...
            JCList<JCVariableDecl> params = md.getParameters();
            md.body = make.Block(0, JCList.of(makeTwrCloseStatement(params.get(0).sym,
        ...
        if (closeResource != null) {
            ...
            closeStatement = make.Exec(make.Apply(JCList.nil(),
                                                  make.Ident(closeResource),
                                                  JCList.of(make.Ident(primaryException),
                                                          resource)
    ...
    private JCStatement makeTwrCloseStatement(Symbol primaryException, JCExpression resource) {
        ...
        JCStatement addSuppressionStatement =
            make.Exec(makeCall(make.Ident(primaryException),
                               names.addSuppressed,
                               JCList.of(make.Ident(catchException))));

        // try { resource.close(); } catch (e) { primaryException.addSuppressed(e); }
        JCBlock tryBlock =
            make.Block(0L, JCList.of(makeResourceCloseInvocation(resource)));
        JCVariableDecl catchExceptionDecl = make.VarDef(catchException, null);
        JCBlock catchBlock = make.Block(0L, JCList.of(addSuppressionStatement));
        JCList<JCCatch> catchClauses = JCList.of(make.Catch(catchExceptionDecl, catchBlock));
    ...
    JCExpression makeOuterThis(DiagnosticPosition pos, TypeSymbol c) {
        JCList<VarSymbol> ots = outerThisStack;
        if (ots.isEmpty()) {
 ...
 *************************************************************************/
    ...
    private MethodSymbol classDollarSym(DiagnosticPosition pos) {
        ...
        if (classDollarSym == null) {
            ...
            try {
                ...
                md.body = make.Block(0, JCList.nil());
    ...
    JCBlock classDollarSymBody(DiagnosticPosition pos, JCMethodDecl md) {
        ...
        JCNewArray newcache = make.NewArray(make.Type(outerCacheClass.type),
                                            JCList.of(make.Literal(INT, 0).setType(syms.intType)),
                                            null);
                ...
                make.Conditional(
                        ...
                        make.Assign(make.Ident(clsym),
                                    makeCall(
                                            ...
                                            JCList.nil())).setType(syms.classLoaderType),
                        make.Ident(clsym)).setType(syms.classLoaderType);

        // returnResult := "{ return Class.forName(param1, false, cl$); }"
        JCList<JCExpression> args = JCList.of(make.Ident(md.params.head.sym),
                                          makeLit(syms.booleanType, 0),
                                          clvalue);
        returnResult = make.Block(0, JCList.of(make.Call(make.App(make.Ident(forNameSym), args))));
        ...
        JCExpression throwExpr =
            makeCall(makeNewClass(syms.noClassDefFoundErrorType,
                                  JCList.nil()),
                     names.initCause,
                     JCList.of(make.Ident(catchParam)));
        rethrow = make.Throw(throwExpr);

        // rethrowStmt := "( $rethrow )"
        JCBlock rethrowStmt = make.Block(0, JCList.of(rethrow));
        ...
        JCStatement tryCatch = make.Try(returnResult,
                                        JCList.of(catchBlock), null);

        return make.Block(0, JCList.of(tryCatch));
    }
    // where
        /** Create an attributed tree of the form left.name(). */
        private JCMethodInvocation makeCall(JCExpression left, Name name, JCList<JCExpression> args) {
    ...
    private JCExpression assertFlagTest(DiagnosticPosition pos) {
        ...
        if (assertDisabledSym == null) {
            ...
            if (currentClass.isInterface()) {
                ...
                JCBlock clinit = make.Block(STATIC, JCList.of(dummy));
 ...
 *************************************************************************/
     ...
     */
    public <T extends JCTree> JCList<T> translate(JCList<T> trees, JCExpression enclOp) {
        JCExpression prevEnclOp = this.enclOp;
        this.enclOp = enclOp;
        JCList<T> res = translate(trees);
        this.enclOp = prevEnclOp;
        return res;
    }
    ...
    /** Visitor method: Translate list of trees.
     */
    public <T extends JCExpression> JCList<T> translate(JCList<T> trees, Type type) {
    ...
    public void visitModuleDef(JCModuleDecl tree) {
        ...
        createInfoClass(JCList.nil(), tree.sym.module_info);
    }

    private void createInfoClass(JCList<JCAnnotation> annots, ClassSymbol c) {
        long flags = Flags.ABSTRACT | Flags.INTERFACE;
    ...
    public void visitClassDef(JCClassDecl tree) {
        ...
        JCList<VarSymbol> prevOuterThisStack = outerThisStack;
        ...
        JCList<JCVariableDecl> fvdefs = freevarDefs(
        ...
        // list `tree.defs'.
        JCList<JCTree> seen = JCList.nil();
        while (tree.defs != seen) {
            JCList<JCTree> unseen = tree.defs;
            for (JCList<JCTree> l = unseen; l.nonEmpty() && l != seen; l = l.tail) {
        ...
        for (JCList<JCVariableDecl> l = fvdefs; l.nonEmpty(); l = l.tail) {
        ...
        result = make_at(tree.pos()).Block(SYNTHETIC, JCList.nil());
    }
    ...
    private void visitEnumDef(JCClassDecl tree) {
        ...
        for (JCList<JCTree> defs = tree.defs;
             defs.nonEmpty();
             defs=defs.tail) {
        ...
        Symbol valuesSym = lookupMethod(tree.pos(), names.values,
                                        tree.type, JCList.nil());
        JCList<JCStatement> valuesBody;
        if (useClone()) {
            ...
            valuesBody = JCList.of(make.Return(valuesResult));
        } else {
            ...
            JCNewArray resultArray = make.NewArray(make.Type(types.erasure(tree.type)),
                                  JCList.of(make.Select(make.Ident(valuesVar), syms.lengthVar)),
                                  null);
            ...
            if (systemArraycopyMethod == null) {
                systemArraycopyMethod =
                    new MethodSymbol(PUBLIC | STATIC,
                                     ...
                                     new MethodType(JCList.of(syms.objectType,
                                                            syms.intType,
                                                            syms.objectType,
                                                            syms.intType,
                                                            syms.intType),
            ...
            JCStatement copy =
                make.Exec(make.App(make.Select(make.Ident(syms.systemType.tsym),
                                               systemArraycopyMethod),
                          JCList.of(make.Ident(valuesVar), make.Literal(0),
            ...
            valuesBody = JCList.of(decl, copy, ret);
        }
        ...
        JCStatement enum_ValueOf =
            make.Return(make.TypeCast(tree.sym.type,
                                      makeCall(make.Ident(syms.enumSym),
                                               names.valueOf,
                                               JCList.of(e_class, nameVal))));
        JCMethodDecl valueOf = make.MethodDef(valueOfSym,
                                           make.Block(0, JCList.of(enum_ValueOf)));
    ...
    private void visitMethodDefInternal(JCMethodDecl tree) {
        if (tree.name == names.init &&
            ...
            JCList<VarSymbol> prevOuterThisStack = outerThisStack;
            JCList<VarSymbol> fvs = freevars(currentClass);
            JCVariableDecl otdef = null;
            if (currentClass.hasOuterInstance())
                otdef = outerThisDef(tree.pos, m);
            JCList<JCVariableDecl> fvdefs = freevarDefs(tree.pos, fvs, m, PARAMETER);
            ...
            JCList<JCStatement> added = JCList.nil();
            if (fvs.nonEmpty()) {
                JCList<Type> addedargtypes = JCList.nil();
                for (JCList<VarSymbol> l = fvs; l.nonEmpty(); l = l.tail) {
            ...
            JCList<JCStatement> stats = translate(tree.body.stats.tail);
    ...
    public void visitNewClass(JCNewClass tree) {
        ...
        JCList<Type> argTypes = tree.constructor.type.getParameterTypes();
...
//where
    ...
    public void visitAssert(JCAssert tree) {
        ...
        if (!tree.cond.type.isTrue()) {
            ...
            JCList<JCExpression> exnArgs = (tree.detail == null) ?
                JCList.nil() : JCList.of(translate(tree.detail));
    ...
    public void visitApply(JCMethodInvocation tree) {
        ...
        JCList<Type> argtypes = meth.type.getParameterTypes();
        ...
        if (meth.name==names.init) {
            ...
            if ((c.flags_field&ENUM) != 0 || c.getQualifiedName() == names.java_lang_Enum) {
                JCList<JCVariableDecl> params = currentMethodDef.params;
    ...
    JCList<JCExpression> boxArgs(JCList<Type> parameters, JCList<JCExpression> _args, Type varargsElement) {
        JCList<JCExpression> args = _args;
    ...
    public void visitAssign(JCAssign tree) {
        ...
        if (tree.lhs.hasTag(APPLY)) {
            ...
            app.args = JCList.of(tree.rhs).prependList(app.args);
            result = app;
        } else {
            result = tree;
        }
    ...
    public void visitAssignop(final JCAssignOp tree) {
        ...
        if (boxingReq || depScanner.dependencyFound) {
            ...
            JCTree newTree = abstractLval(tree.lhs, lhs -> {
                JCTreeTag newTag = tree.getTag().noAssignOp();
        ...
        if (tree.lhs.hasTag(APPLY)) {
            ...
            JCExpression rhs = tree.operator.opcode == string_add
              ? makeString(tree.rhs)
              : tree.rhs;
            app.args = JCList.of(rhs).prependList(app.args);
            result = app;
        } else {
            result = tree;
        }
    ...
    JCExpression lowerBoxedPostop(final JCUnary tree) {
        ...
        return abstractLval(tree.arg, tmp1 -> abstractRval(tmp1, tree.arg.type, tmp2 -> {
            JCTreeTag opcode = (tree.hasTag(POSTINC))
                ? PLUS_ASG : MINUS_ASG;
    ...
    public void visitUnary(JCUnary tree) {
        ...
        if (isUpdateOperator && !tree.arg.type.isPrimitive()) {
            ...
            case PREDEC:            // -- e
                    // translate to e -= 1
                {
                    JCTree.JCTreeTag opcode = (tree.hasTag(PREINC))
                        ? PLUS_ASG : MINUS_ASG;
    ...
    public void visitBinary(JCBinary tree) {
        JCList<Type> formals = tree.operator.type.getParameterTypes();
    ...
    public void visitForeachLoop(JCEnhancedForLoop tree) {
        ...
        private void visitArrayForeachLoop(JCEnhancedForLoop tree) {
            ...
            JCList<JCStatement> loopinit = JCList.of(arraycachedef, lencachedef, indexdef);
            ...
            JCBlock body = make.
                Block(0, JCList.of(loopvardef, tree.body));
        ...
        private void visitIterableForeachLoop(JCEnhancedForLoop tree) {
            ...
            JCBlock body = make.Block(0, JCList.of(indexDef, tree.body));
            body.endpos = TreeInfo.endPos(tree.body);
            result = translate(make.
                ForLoop(JCList.of(init),
                        cond,
                        JCList.nil(),
                        body));
            patchTargets(body, tree, result);
    ...
    public JCTree visitStringSwitch(JCSwitch tree) {
        JCList<JCCase> caseList = tree.getCases();
        ...
        if (alternatives == 0) { // Strange but legal possibility
            ...
            JCMethodInvocation hashCodeCall = makeCall(make.Ident(dollar_s),
                                                       names.hashCode,
                                                       JCList.nil()).setType(syms.intType);
            ...
            for(Map.Entry<Integer, Set<String>> entry : hashToString.entrySet()) {
                ...
                for(String caseLabel : stringsWithHashCode ) {
                    JCMethodInvocation stringEqualsCall = makeCall(make.Ident(dollar_s),
                                                                   names.equals,
                                                                   JCList.of(make.Literal(caseLabel)));
    ...
    public void visitNewArray(JCNewArray tree) {
        ...
        for (JCList<JCExpression> t = tree.dims; t.tail != null; t = t.tail)
 ...
 *************************************************************************/
     ...
     */
    public JCList<JCTree> translateTopLevelClass(Env<com.flint.tools.flintc.comp.AttrContext> env, JCTree cdef, TreeMaker make) {
