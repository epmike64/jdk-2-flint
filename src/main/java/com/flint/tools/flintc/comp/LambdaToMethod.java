...
import com.flint.tools.flintc.jvm.ClassFile;
...
import com.flint.tools.flintc.code.Symbol.DynamicMethodSymbol;
...
import com.flint.tools.flintc.comp.LambdaToMethod.LambdaAnalyzerPreprocessor.*;
...
import static com.flint.tools.flintc.comp.LambdaToMethod.LambdaSymbolKind.*;
...
public class LambdaToMethod extends TreeTranslator {
    ...
    private class KlassInfo {
       ...
       /**
        ...
        private KlassInfo(JCClassDecl clazz) {
            ...
            MethodType type = new MethodType(JCList.of(syms.serializedLambdaType), syms.objectType,
    ...
    <T extends JCTree> JCList<T> translate(JCList<T> trees, TranslationContext<?> newContext) {
    ...
    public void visitClassDef(JCClassDecl tree) {
        ...
        try {
            ...
            JCList<JCTree> newMethods = kInfo.appendedMethodList.toList();
    ...
    public void visitLambda(JCLambda tree) {
        ...
        JCList<JCExpression> indy_args = translate(syntheticInits.toList(), localContext.prev);
    ...
    // where
        ...
        private void apportionTypeAnnotations(JCLambda tree,
                                              Supplier<JCList<Attribute.TypeCompound>> source,
                                              Consumer<JCList<Attribute.TypeCompound>> owner,
                                              Consumer<JCList<Attribute.TypeCompound>> lambda) {
    ...
    public void visitReference(JCMemberReference tree) {
        ...
        JCList<JCExpression> indy_args = init==null? JCList.nil() : translate(JCList.of(init), localContext.prev);
    ...
    private JCBlock makeLambdaExpressionBody(JCExpression expr, JCMethodDecl lambdaMethodDecl) {
        ...
        try {
            if (isTarget_void) {
                ...
                return make.Block(0, JCList.of(stat));
                ...
                return make.at(retExpr).Block(0, JCList.of(make.Return(retExpr)));
            }
        } finally {
            make.at(prevPos);
        }
        ...
        class LambdaBodyTranslator extends TreeTranslator {
            ...
            public void visitReturn(JCReturn tree) {
                ...
                if (isTarget_void && !isLambda_void) {
                    ...
                    result = make.Block(0, JCList.of(varDef, make.Return(null)));
    ...
    private JCMethodDecl makeDeserializeMethod(Symbol kSym) {
        ...
        for (Map.Entry<String, ListBuffer<JCStatement>> entry : kInfo.deserializeCases.entrySet()) {
            ...
            breaks.add(br);
            JCList<JCStatement> stmts = entry.getValue().append(br).toList();
        ...
        JCBlock body = make.Block(0L, JCList.of(
                sw,
                make.Throw(makeNewClass(
                    syms.illegalArgumentExceptionType,
                    JCList.of(make.Literal("Invalid lambda deserialization"))))));
        JCMethodDecl deser = make.MethodDef(make.Modifiers(kInfo.deserMethodSym.flags()),
                        ...
                        JCList.nil(),
                        JCList.of(make.VarDef(kInfo.deserParamSym, null)),
                        JCList.nil(),
                        body,
                        null);
     ...
     */
    JCNewClass makeNewClass(Type ctype, JCList<JCExpression> args, Symbol cons) {
    ...
    JCNewClass makeNewClass(Type ctype, JCList<JCExpression> args) {
        return makeNewClass(ctype, args,
                rs.resolveConstructor(null, attrEnv, ctype, TreeInfo.types(args), JCList.nil()));
     }

    private void addDeserializationCase(int implMethodKind, Symbol refSym, Type targetType, MethodSymbol samSym,
                                        DiagnosticPosition pos, JCList<Object> staticArgs, MethodType indyType) {
        ...
        for (Type t : indyType.getParameterTypes()) {
            JCList<JCExpression> indexAsArg = new ListBuffer<JCExpression>().append(make.Literal(i)).toList();
            JCList<Type> argTypes = new ListBuffer<Type>().append(syms.intType).toList();
    ...
    private JCExpression eqTest(Type argType, JCExpression arg1, JCExpression arg2) {
        JCBinary testExpr = make.Binary(JCTree.JCTreeTag.EQ, arg1, arg2);
        testExpr.operator = operators.resolveBinary(testExpr, JCTree.JCTreeTag.EQ, argType, argType);
        testExpr.setType(syms.booleanType);
        return testExpr;
    }

    private JCExpression deserTest(JCExpression prev, String func, String lit) {
        MethodType eqmt = new MethodType(JCList.of(syms.objectType), syms.booleanType, JCList.nil(), syms.methodClass);
        Symbol eqsym = rs.resolveQualifiedMethod(null, attrEnv, syms.objectType, names.equals, JCList.of(syms.objectType), JCList.nil());
        JCMethodInvocation eqtest = make.Apply(
                JCList.nil(),
                make.Select(deserGetter(func, syms.stringType), eqsym).setType(eqmt),
                JCList.of(make.Literal(lit)));
        eqtest.setType(syms.booleanType);
        JCBinary compound = make.Binary(JCTree.JCTreeTag.AND, prev, eqtest);
        compound.operator = operators.resolveBinary(compound, JCTree.JCTreeTag.AND, syms.booleanType, syms.booleanType);
        compound.setType(syms.booleanType);
        return compound;
    }

    private JCExpression deserGetter(String func, Type type) {
        return deserGetter(func, type, JCList.nil(), JCList.nil());
    }

    private JCExpression deserGetter(String func, Type type, JCList<Type> argTypes, JCList<JCExpression> args) {
        MethodType getmt = new MethodType(argTypes, type, JCList.nil(), syms.methodClass);
        Symbol getsym = rs.resolveQualifiedMethod(null, attrEnv, syms.serializedLambdaType, names.fromString(func), argTypes, JCList.nil());
        return make.Apply(
                    JCList.nil(),
     ...
     */
    private JCList<JCExpression> convertArgs(Symbol meth, JCList<JCExpression> args, Type varargsElement) {
       Assert.check(meth.kind == MTH);
       JCList<Type> formals = types.erasure(meth.type).getParameterTypes();
       if (varargsElement != null) {
    ...
    private class MemberReferenceToLambda {
        ...
        VarSymbol addParametersReturnReceiver() {
            ...
            JCList<Type> samPTypes = samDesc.getParameterTypes();
            JCList<Type> descPTypes = tree.getDescriptorType(types).getParameterTypes();
            ...
            JCList<Type> implPTypes = tree.sym.type.getParameterTypes();
        ...
        private JCExpression expressionInvoke(VarSymbol rcvr) {
            ...
            JCExpression apply = make.Apply(JCList.nil(), select,
        ...
        private JCExpression expressionNew() {
            if (tree.kind == ReferenceKind.ARRAY_CTOR) {
                ...
                JCNewArray newArr = make.NewArray(
                        ...
                        JCList.of(make.Ident(params.first())),
                        null);
    ...
    private JCExpression makeMetafactoryIndyCall(TranslationContext<?> context,
                                                 int refKind, Symbol refSym, JCList<JCExpression> indy_args) {
        JCFunctionalExpression tree = context.tree;
        //determine the static bsm args
        MethodSymbol samSym = (MethodSymbol) types.findDescriptorSymbol(tree.type.tsym);
        JCList<Object> staticArgs = JCList.of(
    ...
    private JCExpression makeIndyCall(DiagnosticPosition pos, Type site, Name bsmName,
                                      JCList<Object> staticArgs, MethodType indyType, JCList<JCExpression> indyArgs,
                                      Name methName) {
        int prevPos = make.pos;
        try {
            make.at(pos);
            ...
            Symbol bsm = rs.resolveInternalMethod(pos, attrEnv, site,
                    bsmName, bsm_staticArgs, JCList.nil());
            ...
            JCMethodInvocation proxyCall = make.Apply(JCList.nil(), qualifier, indyArgs);
    ...
    //where
    private JCList<Type> bsmStaticArgToTypes(JCList<Object> args) {
    ...
    class LambdaAnalyzerPreprocessor extends TreeTranslator {
        ...
        private JCList<Frame> frameStack;
         ...
         */
        private JCList<ClassSymbol> typesUnderConstruction;
        ...
        private JCClassDecl analyzeAndPreprocessClass(JCClassDecl tree) {
            frameStack = JCList.nil();
            typesUnderConstruction = JCList.nil();
        ...
        public void visitApply(JCMethodInvocation tree) {
            JCList<ClassSymbol> previousNascentTypes = typesUnderConstruction;
            try {
            ...
            private ClassSymbol currentClass() {
                for (Frame frame : frameStack) {
                    if (frame.tree.hasTag(JCTree.JCTreeTag.CLASSDEF)) {
        ...
        public void visitBlock(JCBlock tree) {
            JCList<Frame> prevStack = frameStack;
            try {
        ...
        private Symbol owner(boolean skipLambda) {
            JCList<Frame> frameStack2 = frameStack;
            ...
            if (isStatic) {
                ...
                if (clinit == null) {
                    ...
                    clinit = makePrivateSyntheticMethod(STATIC,
                            names.clinit,
                            new MethodType(JCList.nil(), syms.voidType,
                                JCList.nil(), syms.methodClass),
                            csym);
                    clinits.put(csym, clinit);
                }
        ...
        class LambdaTranslationContext extends TranslationContext<JCLambda> {
            ...
            JCList<JCVariableDecl> syntheticParams;
        ...
        final class ReferenceTranslationContext extends TranslationContext<JCMemberReference> {
            ...
            boolean interfaceParameterIsIntersectionType() {
                JCList<Type> tl = tree.getDescriptorType(types).getParameterTypes();
