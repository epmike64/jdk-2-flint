...
import com.flint.tools.flintc.resources.CompilerProperties.Warnings;
...
import static com.flint.tools.flintc.code.Flags.SYNCHRONIZED;
 ...
 **************************************************************************/
    ...
    Type checkClassType(DiagnosticPosition pos, Type t, boolean noBounds) {
        ...
        if (noBounds && t.isParameterized()) {
            JCList<Type> args = t.getTypeArguments();
            while (args.nonEmpty()) {
     ...
     */
    JCList<Type> checkRefTypes(JCList<JCExpression> trees, JCList<Type> types) {
        JCList<JCExpression> tl = trees;
        for (JCList<Type> l = types; l.nonEmpty(); l = l.tail) {
    ...
    Type checkMethod(final Type mtype,
        ...
        Type owntype = mtype;
        JCList<Type> formals = owntype.getParameterTypes();
        JCList<Type> nonInferred = sym.type.getParameterTypes();
        ...
        JCList<JCExpression> args = argtrees;
        if (args != null) {
    ...
    //WHERE
        private Type firstIncompatibleTypeArg(Type type) {
            JCList<Type> formals = type.tsym.type.allparams();
            JCList<Type> actuals = type.allparams();
            JCList<Type> args = type.getTypeArguments();
            JCList<Type> forms = type.tsym.type.getTypeArguments();
            ...
            JCList<Type> tvars_cap = types.substBounds(formals,
                                      formals,
            ...
            args = type.getTypeArguments();
            JCList<Type> bounds = bounds_buf.toList();
 ...
 **************************************************************************/
    ...
    /** Visitor method: Validate a JCList of type expressions.
     */
    void validate(JCList<? extends JCTree> trees, Env<AttrContext> env) {
    ...
    class Validator extends JCTree.Visitor {
        ...
        public void visitTypeApply(JCTypeApply tree) {
            if (tree.type.hasTag(CLASS)) {
                JCList<JCExpression> args = tree.arguments;
                JCList<Type> forms = tree.type.tsym.type.getTypeArguments();
        ...
        public void validateTrees(JCList<? extends JCTree> trees, boolean checkRaw, boolean isOuter) {
            for (JCList<? extends JCTree> l = trees; l.nonEmpty(); l = l.tail)
 ...
 **************************************************************************/
    ...
    /** Is given type a subtype of some of the types in given JCList?
     */
    boolean subset(Type t, JCList<Type> ts) {
     ...
     *  some of the types in given JCList?
     */
    boolean intersects(Type t, JCList<Type> ts) {
        for (JCList<Type> l = ts; l.nonEmpty(); l = l.tail)
            if (types.isSubtype(t, l.head) || types.isSubtype(l.head, t)) return true;
        return false;
    }

    /** Add type set to given type JCList, unless it is a subclass of some class
     *  in the JCList.
     */
    JCList<Type> incl(Type t, JCList<Type> ts) {
        return subset(t, ts) ? ts : excl(t, ts).prepend(t);
    }

    /** Remove type set from type set JCList.
     */
    JCList<Type> excl(Type t, JCList<Type> ts) {
        if (ts.isEmpty()) {
            return ts;
        } else {
            JCList<Type> ts1 = excl(t, ts.tail);
    ...
    /** Form the union of two type set JCLists.
     */
    JCList<Type> union(JCList<Type> ts1, JCList<Type> ts2) {
        JCList<Type> ts = ts1;
        for (JCList<Type> l = ts2; l.nonEmpty(); l = l.tail)
            ts = incl(l.head, ts);
        return ts;
    }

    /** Form the difference of two type JCLists.
     */
    JCList<Type> diff(JCList<Type> ts1, JCList<Type> ts2) {
        JCList<Type> ts = ts1;
        for (JCList<Type> l = ts2; l.nonEmpty(); l = l.tail)
            ts = excl(l.head, ts);
        return ts;
    ...
    /** Form the intersection of two type JCLists.
     */
    public JCList<Type> intersect(JCList<Type> ts1, JCList<Type> ts2) {
        JCList<Type> ts = JCList.nil();
        for (JCList<Type> l = ts1; l.nonEmpty(); l = l.tail)
    ...
    /** Is exc handled by given exception JCList?
     */
    boolean isHandled(Type exc, JCList<Type> handled) {
        return isUnchecked(exc) || subset(exc, handled);
    }

    /** Return all exceptions in thrown JCList that are not in handled JCList.
     *  @param thrown     The JCList of thrown exceptions.
     *  @param handled    The JCList of handled exceptions.
     */
    JCList<Type> unhandled(JCList<Type> thrown, JCList<Type> handled) {
        JCList<Type> unhandled = JCList.nil();
 ...
 **************************************************************************/
    ...
    void checkOverride(JCTree tree,
        ...
        JCList<Type> mtvars = mt.getTypeArguments();
        JCList<Type> otvars = ot.getTypeArguments();
        ...
        JCList<Type> otthrown = types.subst(ot.getThrownTypes(), otvars, mtvars);
        JCList<Type> unhandledErased = unhandled(mt.getThrownTypes(), types.erasure(otthrown));
        JCList<Type> unhandledUnerased = unhandled(mt.getThrownTypes(), otthrown);
    ...
    // where
        private boolean isDeprecatedOverrideIgnorable(MethodSymbol m, ClassSymbol origin) {
            ...
            if (mc != null && ((mc.flags() & INTERFACE) != 0)) {
                JCList<Type> intfs = types.interfaces(origin.type);
    ...
    private Symbol firstDirectIncompatibility(DiagnosticPosition pos, Type t1, Type t2, Type site) {
        for (Symbol s1 : t1.tsym.members().getSymbols(NON_RECURSIVE)) {
            ...
            for (Symbol s2 : t2.tsym.members().getSymbolsByName(s1.name)) {
                ...
                Type st2 = types.memberType(t2, s2);
                if (types.overrideEquivalent(st1, st2)) {
                    JCList<Type> tvars1 = st1.getTypeArguments();
                    JCList<Type> tvars2 = st2.getTypeArguments();
    ...
    public void checkClassOverrideEqualsAndHashIfNeeded(DiagnosticPosition pos,
        ...
        if (someClass.isAnonymous()) {
            JCList<Type> interfaces =  types.interfaces(someClass.type);
    ...
    class CycleChecker extends TreeScanner {
        ...
        JCList<Symbol> seenClasses = JCList.nil();
        ...
        public void visitClassDef(JCClassDecl tree) {
            JCList<JCTree> supertypes = JCList.nil();
        ...
        void checkClass(DiagnosticPosition pos, Symbol c, JCList<JCTree> supertypes) {
    ...
    void checkNonCyclic(DiagnosticPosition pos, TypeVar t) {
        checkNonCyclic1(pos, t, JCList.nil());
    }

    private void checkNonCyclic1(DiagnosticPosition pos, Type t, JCList<TypeVar> seen) {
        final TypeVar tv;
            ...
            try {
                ...
                if (c.type.hasTag(CLASS)) {
                    ...
                    if (clazz.interfaces_field != null)
                        for (JCList<Type> l=clazz.interfaces_field; l.nonEmpty(); l=l.tail)
    ...
    private void noteCyclic(DiagnosticPosition pos, ClassSymbol c) {
        ...
        for (JCList<Type> l=types.interfaces(c.type); l.nonEmpty(); l=l.tail)
    ...
    //where
        ...
        void checkImplementations(JCTree tree, ClassSymbol origin, ClassSymbol ic) {
            for (JCList<Type> l = types.closure(ic.type); l.nonEmpty(); l = l.tail) {
    ...
    void checkCompatibleSupertypes(DiagnosticPosition pos, Type c) {
        JCList<Type> supertypes = types.interfaces(c);
        Type supertype = types.supertype(c);
        if (supertype.hasTag(CLASS) &&
            (supertype.tsym.flags() & ABSTRACT) != 0)
            supertypes = supertypes.prepend(supertype);
        for (JCList<Type> l = supertypes; l.nonEmpty(); l = l.tail) {
            if (!l.head.getTypeArguments().isEmpty() &&
                !checkCompatibleAbstracts(pos, l.head, l.head, c))
                return;
            for (JCList<Type> m = supertypes; m != l; m = m.tail)
    ...
    void checkOverrideClashes(DiagnosticPosition pos, Type site, MethodSymbol sym) {
        ...
        JCList<MethodSymbol> potentiallyAmbiguousJCList = JCList.nil();
        ...
        for (Symbol m1 : types.membersClosure(site, false).getSymbolsByName(sym.name, cf)) {
            if (!sym.overrides(m1, site.tsym, types, false)) {
                ...
                if (!overridesAny) {
                    potentiallyAmbiguousJCList = potentiallyAmbiguousJCList.prepend((MethodSymbol)m1);
                }
                continue;
            }
            ...
            if (m1 != sym) {
                overridesAny = true;
                potentiallyAmbiguousJCList = JCList.nil();
            }
        ...
        if (!overridesAny) {
            for (MethodSymbol m: potentiallyAmbiguousJCList) {
    ...
    void checkDefaultMethodClashes(DiagnosticPosition pos, Type site) {
        ...
        for (Symbol m : types.membersClosure(site, false).getSymbols(dcf)) {
            ...
            JCList<MethodSymbol> prov = types.interfaceCandidates(site, (MethodSymbol)m);
            if (prov.size() > 1) {
    ...
    void checkPotentiallyAmbiguousOverloads(DiagnosticPosition pos, Type site,
        ...
        if (msym1 != msym2 &&
            ...
            JCList<Type> args1 = rs.adjustArgs(mt1.getParameterTypes(), msym1, maxLength, true);
            JCList<Type> args2 = rs.adjustArgs(mt2.getParameterTypes(), msym2, maxLength, true);
     ...
     *  the same parameterized interface with two different argument JCLists.
     *  @param pos          Position to be used for error reporting.
...
//where
        ...
        void checkClassBounds(DiagnosticPosition pos,
            ...
            for (JCList<Type> l = types.interfaces(type); l.nonEmpty(); l = l.tail) {
                Type it = l.head;
                Type oldit = seensofar.put(it.tsym, it);
                if (oldit != null) {
                    JCList<Type> oldparams = oldit.allparams();
                    JCList<Type> newparams = it.allparams();
 ...
 **************************************************************************/
     ...
     */
    public void validateAnnotations(JCList<JCAnnotation> annotations, Symbol s) {
     ...
     */
    public void validateTypeAnnotations(JCList<JCAnnotation> annotations, boolean isTypeParameter) {
        for (JCAnnotation a : annotations)
    ...
    public void validateRepeatable(TypeSymbol s, Attribute.Compound repeatable, DiagnosticPosition pos) {
        ...
        Type t = null;
        JCList<Pair<MethodSymbol,Attribute>> l = repeatable.values;
        if (!l.isEmpty()) {
    ...
    protected boolean isTypeAnnotation(JCAnnotation a, boolean isTypeParameter) {
        JCList<Attribute> targets = typeAnnotations.annotationTargets(a.annotationType.type.tsym);
        return (targets == null) ?
                false :
                targets.stream()
    ...
    private boolean validateAnnotation(JCAnnotation a) {
        ...
        JCList<Name> missingDefaults = JCList.nil();
        ...
        for (final JCImport imp : toplevel.getImports()) {
            ...
            if (!checkTypeContainsImportableElement(site, site, toplevel.packge, select.name, new HashSet<Symbol>())) {
                log.error(imp.pos(), "cant.resolve.location",
                          KindName.STATIC,
                          select.name, JCList.<Type>nil(), JCList.<Type>nil(),
    ...
    public void checkLeaksNotAccessible(Env<AttrContext> env, JCClassDecl check) {
        ...
        private void checkVisible(DiagnosticPosition pos, Symbol what, PackageSymbol inPackage, boolean inSuperType) {
            ...
            if (whatPackage.modle != inPackage.modle && whatPackage.modle != syms.java_base) {
                ...
                JCList<ModuleSymbol> todo = JCList.of(inPackage.modle);
