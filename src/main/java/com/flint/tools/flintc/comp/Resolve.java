...
import com.flint.tools.flintc.api.Formattable.LocalizedString;
...
import com.flint.tools.flintc.comp.Resolve.MethodResolutionContext.Candidate;
import com.flint.tools.flintc.comp.Resolve.MethodResolutionDiagHelper.Template;
import com.flint.tools.flintc.comp.Resolve.ReferenceLookupResult.StaticKind;
...
import static com.flint.tools.flintc.code.Flags.STATIC;
...
import static com.flint.tools.flintc.comp.Resolve.MethodResolutionPhase.*;
...
public class Resolve {
    ...
    void reportVerboseResolutionDiagnostic(DiagnosticPosition dpos, Name name, Type site,
                                           JCList<Type> argtypes, JCList<Type> typeargtypes, Symbol bestSoFar) {
        ...
        JCList<Type> argtypes2 = argtypes.map(deferredAttr.new RecoveryDeferredTypeMap(AttrMode.SPECULATIVE, bestSoFar, currentResolutionContext.step));
 ...
 *************************************************************************/
    ...
    Types.SimpleVisitor<Void, Env<AttrContext>> accessibilityChecker =
        ...
        void visit(JCList<Type> ts, Env<AttrContext> env) {
            for (Type t : ts) {
                visit(t, env);
            }
        }
    ...
    Type rawInstantiate(Env<AttrContext> env,
        ...
        // need to inferred.
        JCList<Type> tvars = JCList.nil();
        if (typeargtypes == null) typeargtypes = JCList.nil();
        if (!mt.hasTag(FORALL) && typeargtypes.nonEmpty()) {
            ...
            JCList<Type> formals = pmt.tvars;
            JCList<Type> actuals = typeargtypes;
            while (formals.nonEmpty() && actuals.nonEmpty()) {
                JCList<Type> bounds = types.subst(types.getBounds((TypeVar)formals.head),
            ...
            ForAll pmt = (ForAll) mt;
            JCList<Type> tvars1 = types.newInstances(pmt.tvars);
    ...
    Type checkMethod(Env<AttrContext> env,
        ...
        try {
            ...
            if (env.tree.hasTag(JCTree.JCTreeTag.REFERENCE)) {
    ...
    interface MethodCheck {
         ...
         * most specific check.
         */
        MethodCheck mostSpecificCheck(JCList<Type> actuals);
    }
    ...
    MethodCheck nilMethodCheck = new MethodCheck() {
        public void argumentsAcceptable(Env<AttrContext> env, DeferredAttrContext deferredAttrContext, JCList<Type> argtypes, JCList<Type> formals, Warner warn) {
            //do nothing - method always applicable regardless of actuals
        }

        public MethodCheck mostSpecificCheck(JCList<Type> actuals) {
            return this;
        }
    };
    ...
    abstract class AbstractMethodCheck implements MethodCheck {
        ...
        public void argumentsAcceptable(final Env<AttrContext> env,
            ...
            JCList<JCExpression> trees = TreeInfo.args(callTree);
    ...
    JCList<Type> dummyArgs(int length) {
    ...
    MethodCheck resolveMethodCheck = new AbstractMethodCheck() {
        ...
        private void varargsAccessible(final Env<AttrContext> env, final Type t, final InferenceContext inferenceContext) {
            if (inferenceContext.free(t)) {
                inferenceContext.addFreeTypeListener(JCList.of(t),
    ...
    class MostSpecificCheck implements MethodCheck {
        ...
        MostSpecificCheck(JCList<Type> actuals) {
        ...
        public void argumentsAcceptable(final Env<AttrContext> env,
                                    DeferredAttrContext deferredAttrContext,
                                    JCList<Type> formals1,
                                    JCList<Type> formals2,
                                    Warner warn) {
        ...
        */
        ...
        class MostSpecificCheckContext extends MethodCheckContext {
            ...
            private boolean functionalInterfaceMostSpecific(Type t, Type s, JCTree tree) {
                ...
                final JCList<Type> tTypeParams = tDesc.getTypeArguments();
                final JCList<Type> tTypeParamsNoCapture = tDescNoCapture.getTypeArguments();
                final JCList<Type> sTypeParams = sDesc.getTypeArguments();
                ...
                JCList<Type> tIter = tTypeParams;
                JCList<Type> sIter = sTypeParams;
                ...
                // compare parameters
                JCList<Type> tParams = tDesc.getParameterTypes();
                JCList<Type> tParamsNoCapture = tDescNoCapture.getParameterTypes();
                JCList<Type> sParams = sDesc.getParameterTypes();
            ...
            class MostSpecificFunctionReturnChecker extends DeferredAttr.PolyScanner {
                ...
                public void visitLambda(JCLambda tree) {
                    if (sRet.hasTag(VOID)) {
                        ...
                        JCList<JCExpression> lambdaResults = lambdaResults(tree);
                ...
                private JCList<JCExpression> lambdaResults(JCLambda lambda) {
                    if (lambda.getBodyKind() == JCTree.JCLambda.BodyKind.EXPRESSION) {
                        return JCList.of(asExpr((JCExpression) lambda.body));
                    } else {
 ...
 ****************************************************************************/
    ...
    Symbol findField(Env<AttrContext> env,
        ...
        for (JCList<Type> l = types.interfaces(c.type);
     ...
     *  @param useVarargs Box trailing arguments into an array for varargs.
     */
    Symbol mostSpecific(JCList<Type> argtypes, Symbol m1,
                        Symbol m2,
                        Env<AttrContext> env,
                        final Type site,
                        boolean useVarargs) {
    ...
    //where
    private boolean signatureMoreSpecific(JCList<Type> actuals, Env<AttrContext> env, Type site, Symbol m1, Symbol m2, boolean useVarargs) {
        noteWarner.clear();
    ...
    JCList<Type> adjustArgs(JCList<Type> args, Symbol msym, int length, boolean allowVarargs) {
        if ((msym.flags() & VARARGS) != 0 && allowVarargs) {
            Type varargsElem = types.elemtype(args.last());
            if (varargsElem == null) {
                Assert.error("Bad varargs = " + args.last() + " " + msym);
            }
            JCList<Type> newArgs = args.reverse().tail.prepend(varargsElem).reverse();
    ...
    private Symbol findMethod(Env<AttrContext> env,
        ...
        JCList<Type>[] itypes = (JCList<Type>[])new JCList[] { JCList.<Type>nil(), JCList.<Type>nil() };
    ...
    Iterable<TypeSymbol> superclasses(final Type intype) {
        return () -> new Iterator<TypeSymbol>() {
            ...
            JCList<TypeSymbol> seen = JCList.nil();
    ...
    private final RecoveryLoadClass doRecoveryLoadClass = new RecoveryLoadClass() {
        @Override public Symbol loadClass(Env<AttrContext> env, Name name) {
            JCList<Name> candidates = Convert.classCandidates(name);
 ...
 ****************************************************************************/
    ...
    Symbol accessBase(Symbol sym,
                  ...
                  Name name,
                  boolean qualified) {
        return accessInternal(sym, pos, location, site, name, qualified, JCList.nil(), null, basicLogResolveHelper);
    }
    ...
    interface LogResolveHelper {
        boolean resolveDiagnosticNeeded(Type site, JCList<Type> argtypes, JCList<Type> typeargtypes);
        JCList<Type> getArgumentTypes(ResolveError errSym, Symbol accessedSym, Name name, JCList<Type> argtypes);
    }

    LogResolveHelper basicLogResolveHelper = new LogResolveHelper() {
        public boolean resolveDiagnosticNeeded(Type site, JCList<Type> argtypes, JCList<Type> typeargtypes) {
            return !site.isErroneous();
        }
        public JCList<Type> getArgumentTypes(ResolveError errSym, Symbol accessedSym, Name name, JCList<Type> argtypes) {
            return argtypes;
        }
    };
 ...
 ****************************************************************************/
    ...
    Symbol resolveQualifiedMethod(DiagnosticPosition pos, Env<AttrContext> env,
                                  Type site, Name name, JCList<Type> argtypes,
                                  JCList<Type> typeargtypes) {
        return resolveQualifiedMethod(pos, env, site.tsym, site, name, argtypes, typeargtypes);
    }
    Symbol resolveQualifiedMethod(DiagnosticPosition pos, Env<AttrContext> env,
                                  Symbol location, Type site, Name name, JCList<Type> argtypes,
                                  JCList<Type> typeargtypes) {
    ...
    Symbol findConstructor(DiagnosticPosition pos, Env<AttrContext> env,
                              Type site, JCList<Type> argtypes,
                              JCList<Type> typeargtypes,
                              boolean allowBoxing,
                              boolean useVarargs) {
        ...
        for (final Symbol sym : tsym.members().getSymbolsByName(names.init)) {
            ...
            if (sym.kind == MTH &&
                (sym.flags_field & SYNTHETIC) == 0) {
                    JCList<Type> oldParams = sym.type.hasTag(FORALL) ?
    ...
    Symbol getMemberReference(DiagnosticPosition pos,
        ...
        ReferenceLookupHelper lookupHelper = makeReferenceLookupHelper(
                referenceTree, site, name, JCList.nil(), null, VARARITY);
     ...
     * the resulting symbol is delegated to an helper class {@link Resolve.ReferenceChooser}.
     *
     ...
     * of the lookup (see {@link Resolve.ReferenceLookupResult.StaticKind}).
     */
    ...
    abstract class LookupHelper {
        ...
        LookupHelper(Name name, Type site, JCList<Type> argtypes, JCList<Type> typeargtypes, MethodResolutionPhase maxPhase) {
            this.name = name;
            this.site = site;
    ...
    abstract class BasicLookupHelper extends LookupHelper {
        ...
        BasicLookupHelper(Name name, Type site, JCList<Type> argtypes, JCList<Type> typeargtypes) {
            this(name, site, argtypes, typeargtypes, MethodResolutionPhase.VARARITY);
        }

        BasicLookupHelper(Name name, Type site, JCList<Type> argtypes, JCList<Type> typeargtypes, MethodResolutionPhase maxPhase) {
    ...
    abstract class ReferenceLookupHelper extends LookupHelper {
        ...
        ReferenceLookupHelper(JCMemberReference referenceTree, Name name, Type site,
                              JCList<Type> argtypes, JCList<Type> typeargtypes, MethodResolutionPhase maxPhase) {
            super(name, site, argtypes, typeargtypes, maxPhase);
    ...
    class ArrayConstructorReferenceLookupHelper extends ReferenceLookupHelper {
        ...
        ArrayConstructorReferenceLookupHelper(JCMemberReference referenceTree, Type site, JCList<Type> argtypes,
                                              JCList<Type> typeargtypes, MethodResolutionPhase maxPhase) {
        ...
        protected Symbol lookup(Env<AttrContext> env, MethodResolutionPhase phase) {
            ...
            arrayConstr.type = new MethodType(JCList.of(syms.intType), site, JCList.nil(), syms.methodClass);
    ...
    class ConstructorReferenceLookupHelper extends ReferenceLookupHelper {
        ...
        ConstructorReferenceLookupHelper(JCMemberReference referenceTree, Type site, JCList<Type> argtypes,
                                         JCList<Type> typeargtypes, MethodResolutionPhase maxPhase) {
            super(referenceTree, names.init, site, argtypes, typeargtypes, maxPhase);
            if (site.isRaw()) {
    ...
    //where
    private JCList<Type> pruneInterfaces(Type t) {
 ...
 ****************************************************************************/
    ...
    public Object methodArguments(JCList<Type> argtypes) {
    ...
    class SymbolNotFoundError extends ResolveError {
        ...
        JCDiagnostic getDiagnostic(JCDiagnostic.DiagnosticType dkind,
                ...
                JCList<Type> argtypes,
                JCList<Type> typeargtypes) {
            argtypes = argtypes == null ? JCList.nil() : argtypes;
            typeargtypes = typeargtypes == null ? JCList.nil() : typeargtypes;
            if (name == names.error)
                return null;
        ...
        //where
        private Object args(JCList<Type> args) {
    ...
    class InapplicableSymbolsError extends InapplicableSymbolError {
        ...
        //where
            ...
            private JCList<JCDiagnostic> candidateDetails(Map<Symbol, JCDiagnostic> candidatesMap, Type site) {
                JCList<JCDiagnostic> details = JCList.nil();
                for (Map.Entry<Symbol, JCDiagnostic> _entry : candidatesMap.entrySet()) {
    ...
    class DiamondError extends InapplicableSymbolError {
        ...
        JCDiagnostic getDiagnostic(DiagnosticType dkind, DiagnosticPosition pos,
                                   Symbol location, Type site, Name name, JCList<Type> argtypes, JCList<Type> typeargtypes) {
    ...
    class AmbiguityError extends ResolveError {
        ...
        JCList<Symbol> ambiguousSyms = JCList.nil();
        ...
        private JCList<Symbol> flatten(Symbol sym) {
            if (sym.kind == AMBIGUOUS) {
                return ((AmbiguityError)sym.baseSymbol()).ambiguousSyms;
            } else {
                return JCList.of(sym);
            }
        }
        ...
        JCDiagnostic getDiagnostic(JCDiagnostic.DiagnosticType dkind,
                ...
                JCList<Type> argtypes,
                JCList<Type> typeargtypes) {
            JCList<Symbol> diagSyms = ambiguousSyms.reverse();
        ...
        Symbol mergeAbstracts(Type site) {
            JCList<Symbol> ambiguousInOrder = ambiguousSyms.reverse();
    ...
    class BadVarargsMethod extends ResolveError {
        ...
        @Override
        JCDiagnostic getDiagnostic(DiagnosticType dkind, DiagnosticPosition pos, Symbol location, Type site, Name name, JCList<Type> argtypes, JCList<Type> typeargtypes) {
    ...
    static class MethodResolutionDiagHelper {
        ...
        static final Template inferArgMismatchTemplate = new Template(MethodCheckDiag.ARG_MISMATCH.regex(), skip, skip) {
            ...
            boolean matches(Object o) {
                ...
                JCList<Type> tvars = (JCList<Type>)d.getArgs()[0];
                return !containsAny(d, tvars);
            }

            BiPredicate<Object, JCList<Type>> containsPredicate = (o, ts) -> {
            ...
            boolean containsAny(JCDiagnostic d, JCList<Type> ts) {
    ...
    final JCList<MethodResolutionPhase> methodResolutionSteps = JCList.of(BASIC, BOX, VARARITY);
    ...
    class MethodResolutionContext {
        ...
        private JCList<Candidate> candidates = JCList.nil();
