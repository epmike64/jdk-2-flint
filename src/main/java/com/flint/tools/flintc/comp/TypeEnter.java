...
import com.flint.tools.flintc.code.Scope.ImportFilter;
...
import static com.flint.tools.flintc.code.TypeTag.ERROR;
...
public class TypeEnter implements Completer {
    ...
    /* Verify Imports:
     */
    protected void ensureImportsChecked(JCList<JCCompilationUnit> trees) {
 ...
 *********************************************************************/
    ...
    public void complete(Symbol sym) throws CompletionFailure {
        ...
        try {
            ...
            JCList<Env<AttrContext>> queue;

            dependencies.push((ClassSymbol) sym, CompletionCause.MEMBER_ENTER);
            try {
                queue = completeClass.completeEnvs(JCList.of(typeEnvs.get((ClassSymbol) sym)));
            } finally {
                dependencies.pop();
            }
    ...
    abstract class Phase {
        ...
        public final JCList<Env<AttrContext>> completeEnvs(JCList<Env<AttrContext>> envs) {
            ...
            if (firstToComplete) {
                JCList<Env<AttrContext>> out = queue.toList();
        ...
        protected void doCompleteEnvs(JCList<Env<AttrContext>> envs) {
    ...
    private abstract class AbstractHeaderPhase extends Phase {
        ...
        protected Env<AttrContext> baseEnv(JCClassDecl tree, Env<AttrContext> env) {
            ...
            if (tree.typarams != null)
                for (JCList<JCTypeParameter> typarams = tree.typarams;
                     typarams.nonEmpty();
        ...
        protected Type modelMissingTypes(Env<AttrContext> env, Type t, final JCExpression tree, final boolean interfaceExpected) {
            ...
            private class Synthesizer extends JCTree.Visitor {
                ...
                JCList<ClassSymbol> synthesizedSymbols = JCList.nil();
                Type result;
                ...
                JCList<Type> visit(JCList<? extends JCTree> trees) {
                ...
                public void visitTypeApply(JCTypeApply tree) {
                    if (!tree.type.hasTag(ERROR)) {
                        ...
                        final JCList<Type> actuals = visit(tree.arguments);
        ...
        protected void attribSuperTypes(Env<AttrContext> env, Env<AttrContext> baseEnv) {
            ...
            JCList<JCExpression> interfaceTrees = tree.implementing;
            ...
            if ((sym.flags_field & ANNOTATION) != 0) {
                ct.interfaces_field = JCList.of(syms.annotationType);
    ...
    private final class HierarchyPhase extends AbstractHeaderPhase implements Completer {
        ...
        protected void runPhase(Env<AttrContext> env) {
            ...
            if (sym.fullname == names.java_lang_Object) {
                if (tree.extending != null) {
                    ...
                    ct.interfaces_field = JCList.nil();
                }
            }
        ...
        public void complete(Symbol sym) throws CompletionFailure {
            ...
            super.doCompleteEnvs(JCList.of(env));
        }
    ...
    private final class MembersPhase extends Phase {
        ...
        private JCList<Env<AttrContext>> todo = JCList.nil();
        ...
        protected void doCompleteEnvs(JCList<Env<AttrContext>> envs) {
            ...
            try {
                while (todo.nonEmpty()) {
                    ...
                    todo = todo.tail;
                    super.doCompleteEnvs(JCList.of(head));
                }
            } finally {
        ...
        protected void runPhase(Env<AttrContext> env) {
            ...
            if ((sym.flags() & INTERFACE) == 0 &&
                ...
                JCList<Type> typarams = JCList.nil();
        ...
        private void addEnumMembers(JCClassDecl tree, Env<AttrContext> env) {
            ...
            JCMethodDecl valueOf = make.
                MethodDef(make.Modifiers(Flags.PUBLIC|Flags.STATIC),
                          ...
                          JCList.nil(),
                          JCList.of(make.VarDef(make.Modifiers(Flags.PARAMETER |
                                                             Flags.MANDATED),
 ...
 ****************************************************************************/
    ...
    JCTree DefaultConstructor(TreeMaker make,
        ...
        init.params = createDefaultConstructorParams(make, baseInit, init,
                argtypes, based);
        JCList<JCVariableDecl> params = make.Params(argtypes, init);
        JCList<JCStatement> stats = JCList.nil();
    ...
    private JCList<VarSymbol> createDefaultConstructorParams(
            TreeMaker make,
            MethodSymbol baseInit,
            MethodSymbol init,
            JCList<Type> argtypes,
            boolean based) {
        JCList<VarSymbol> initParams = null;
        JCList<Type> argTypesList = argtypes;
        if (based) {
        ...
        if (baseInit != null && baseInit.params != null &&
            ...
            initParams = (initParams == null) ? JCList.nil() : initParams;
            JCList<VarSymbol> baseInitParams = baseInit.params;
    ...
    JCExpressionStatement SuperCall(TreeMaker make,
        ...
        JCList<JCExpression> typeargs = typarams.nonEmpty() ? make.Types(typarams) : null;
     ...
     */
    public void markDeprecated(Symbol sym, JCList<JCAnnotation> annotations, Env<AttrContext> env) {
     ...
     **/
    private void handleDeprecatedAnnotations(JCList<JCAnnotation> annotations, Symbol sym) {
        for (JCList<JCAnnotation> al = annotations; !al.isEmpty(); al = al.tail) {
