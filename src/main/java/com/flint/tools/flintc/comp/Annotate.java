...
import static com.flint.tools.flintc.code.Kinds.Kind.PCK;
import static com.flint.tools.flintc.code.Kinds.Kind.VAR;
...
public class Annotate {
    ...
    public JCList<TypeCompound> fromAnnotations(JCList<JCAnnotation> annotations) {
     ...
     */
    public void annotateLater(JCList<JCAnnotation> annotations, Env<AttrContext> localEnv,
    ...
    private <T extends Attribute.Compound> void annotateNow(Symbol toAnnotate,
                                                            JCList<JCAnnotation> withAnnotations, Env<AttrContext> env, boolean typeAnnotations,
                                                            boolean isTypeParam)
    {
        Map<TypeSymbol, ListBuffer<T>> annotated = new LinkedHashMap<>();
        Map<T, DiagnosticPosition> pos = new HashMap<>();

        for (JCList<JCAnnotation> al = withAnnotations; !al.isEmpty(); al = al.tail) {
        ...
        if (typeAnnotations) {
            @SuppressWarnings("unchecked")
            JCList<TypeCompound> attrs = (JCList<TypeCompound>)buf.reverse();
            toAnnotate.appendUniqueTypeAttributes(attrs);
        } else {
            @SuppressWarnings("unchecked")
            JCList<Compound> attrs =  (JCList<Compound>)buf.reverse();
                                                  ...
                                                  Env<AttrContext> env)
    {
        ...
        JCList<Pair<MethodSymbol, Attribute>> elems = attributeAnnotationValues(tree, expectedAnnotationType, env);
                                                          ...
                                                          Env<AttrContext> env)
    {
        ...
        if (a.attribute == null || a.type == null || !(a.attribute instanceof Attribute.TypeCompound)) {
            ...
            JCList<Pair<MethodSymbol,Attribute>> elems =
    ...
    private JCList<Pair<MethodSymbol, Attribute>> attributeAnnotationValues(JCAnnotation a,
                                                                            Type expected, Env<AttrContext> env)
    {
        ...
        JCList<JCExpression> args = a.args;
        ...
        for (JCList<JCExpression> tl = args; tl.nonEmpty(); tl = tl.tail) {
    ...
    private Pair<MethodSymbol, Attribute> attributeAnnotationNameValuePair(JCExpression nameValuePair,
	    ...
	    {
	    }resultType = method.type.getReturnType();
...
//            }
//            for (JCList<JCExpression> l = na.elems; l.nonEmpty(); l=l.tail) {
...
//            }
    ...
    private Attribute getAnnotationArrayValue(Type expectedElementType, JCExpression tree, Env<AttrContext> env) {
        ...
        if (!tree.hasTag(NEWARRAY)) {
            tree = make.at(tree.pos).
                    NewArray(null, JCList.nil(), JCList.of(tree));
        }
     ...
     */
    private <T extends Attribute.Compound> T processRepeatedAnnotations(JCList<T> annotations,
                                                                        AnnotationContext<T> ctx, Symbol on, boolean isTypeParam)
    {
        T firstOccurrence = annotations.head;
        JCList<Attribute> repeated = JCList.nil();
        ...
        int count = 0;
        for (JCList<T> al = annotations; !al.isEmpty(); al = al.tail) {
            count++;
        ...
        if (!repeated.isEmpty()) {
            ...
            if (ctx.isTypeCompound) {
                ...
                */
                ...
                Attribute.TypeCompound at = new Attribute.TypeCompound(targetContainerType, JCList.of(p),
                        ((Attribute.TypeCompound)annotations.head).position);
                ...
                Attribute.Compound c = new Attribute.Compound(targetContainerType, JCList.of(p));
...
//                    expectedType);
    ...
    private <T extends Attribute.Compound> T makeContainerAnnotation(JCList<T> toBeReplaced,
...
//                        manualContainer.first().type.tsym);
     ...
     */
    public void enterTypeAnnotations(JCList<JCAnnotation> annotations, Env<AttrContext> env,
     ...
     */
    public void annotateTypeSecondStage(JCTree tree, JCList<JCAnnotation> annotations, Type storeAt) {
        typeAnnotation(() -> {
            JCList<TypeCompound> compounds = fromAnnotations(annotations);
     ...
     * Apply the annotations to the particular type.
     */
    public void annotateTypeParameterSecondStage(JCTree tree, JCList<JCAnnotation> annotations) {
        typeAnnotation(() -> {
            JCList<TypeCompound> compounds = fromAnnotations(annotations);
            Assert.check(annotations.size() == compounds.size());
        });
    ...
    private class TypeAnnotate extends TreeScanner {
        ...
        public void visitNewArray(JCNewArray tree) {
            enterTypeAnnotations(tree.annotations, env, sym, deferPos, false);
            for (JCList<JCAnnotation> dimAnnos : tree.dimAnnotations)
