...
import com.flint.tools.flintc.code.Kinds.KindName;
 ...
 *************************************************************************/
    ...
    WriteableScope enterScope(Env<AttrContext> env) {
        return (env.tree.hasTag(JCTree.JCTreeTag.CLASSDEF))
 ...
 *************************************************************************/
     ...
     */
    <T extends JCTree> JCList<Type> classEnter(JCList<T> trees, Env<AttrContext> env) {
...
//        Assert.checkNonNull(c.modle, c.sourcefile.toString());
     ...
     */
    public void main(JCList<JCCompilationUnit> trees) {
        complete(trees, null);
    }
     ...
     */
    public void complete(JCList<JCCompilationUnit> trees, ClassSymbol c) {
        annotate.blockAnnotations();
                ...
                if (!modules.modulesInitialized()) {
                    for (JCCompilationUnit cut : trees) {
                        if (cut.getModuleDecl() != null) {
                            ...
                            typeEnter.ensureImportsChecked(JCList.of(cut));
                        }
                    }
                } else {
