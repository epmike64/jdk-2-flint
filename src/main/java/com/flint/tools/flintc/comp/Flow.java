...
import static com.flint.tools.flintc.code.TypeTag.BOOLEAN;
...
public class Flow {
    ...
    public JCList<Type> analyzeLambdaThrownTypes(final Env<AttrContext> env,
    ...
    static abstract class BaseAnalyzer<P extends BaseAnalyzer.PendingExit> extends TreeScanner {
        ...
        enum JumpKind {
            BREAK(JCTree.JCTreeTag.BREAK) {
                @Override
                JCTree getTarget(JCTree tree) {
                    return ((JCBreak)tree).target;
                }
            },
            CONTINUE(JCTree.JCTreeTag.CONTINUE) {
                @Override
                JCTree getTarget(JCTree tree) {
            ...
            final JCTree.JCTreeTag treeTag;

            private JumpKind(JCTreeTag treeTag) {
        ...
        private boolean resolveJump(JCTree tree,
            ...
            JCList<P> exits = pendingExits.toList();
     ...
     *************************************************************************/
        ...
        void scanDef(JCTree tree) {
            scanStat(tree);
            if (tree != null && tree.hasTag(JCTree.JCTreeTag.BLOCK) && !alive) {
                log.error(tree.pos(),
         ...
         */
        void scanStats(JCList<? extends JCStatement> trees) {
            if (trees != null)
                for (JCList<? extends JCStatement> l = trees; l.nonEmpty(); l = l.tail)
                    scanStat(l.head);
        }
        ...
        public void visitClassDef(JCClassDecl tree) {
            ...
            ListBuffer<Flow.BaseAnalyzer.PendingExit> pendingExitsPrev = pendingExits;
            Lint lintPrev = lint;
        ...
        public void visitMethodDef(JCMethodDecl tree) {
            ...
            try {
                ...
                JCList<PendingExit> exits = pendingExits.toList();
                pendingExits = new ListBuffer<>();
                while (exits.nonEmpty()) {
                    Flow.BaseAnalyzer.PendingExit exit = exits.head;
                    exits = exits.tail;
        ...
        public void visitDoLoop(JCDoWhileLoop tree) {
            ListBuffer<Flow.BaseAnalyzer.PendingExit> prevPendingExits = pendingExits;
            pendingExits = new ListBuffer<>();
            scanStat(tree.body);
        ...
        public void visitTry(JCTry tree) {
            ...
            if (tree.finalizer != null) {
                ListBuffer<Flow.BaseAnalyzer.PendingExit> exits = pendingExits;
        ...
        public void visitLambda(JCLambda tree) {
            ...
            ListBuffer<Flow.BaseAnalyzer.PendingExit> prevPending = pendingExits;
    ...
    class FlowAnalyzer extends BaseAnalyzer<FlowAnalyzer.FlowPendingExit> {
         ...
         *  complete normally.
         */
        HashMap<Symbol, JCList<Type>> preciseRethrowTypes;
     ...
     *************************************************************************/
        ...
        public void visitClassDef(JCClassDecl tree) {
            ...
            JCList<Type> thrownPrev = thrown;
            JCList<Type> caughtPrev = caught;
        ...
        public void visitMethodDef(JCMethodDecl tree) {
            ...
            JCList<Type> caughtPrev = caught;
            JCList<Type> mthrown = tree.sym.type.getThrownTypes();
            Lint lintPrev = lint;
            ...
            try {
                ...
                JCList<FlowPendingExit> exits = pendingExits.toList();
                pendingExits = new ListBuffer<>();
                while (exits.nonEmpty()) {
        ...
        public void visitTry(JCTry tree) {
            ...
            thrown = JCList.nil();
            for (JCList<JCCatch> l = tree.catchers; l.nonEmpty(); l = l.tail) {
                JCList<JCExpression> subClauses = TreeInfo.isMultiCatch(l.head) ?
                        ((JCTypeUnion)l.head.param.vartype).alternatives :
                        JCList.of(l.head.param.vartype);
            ...
            for (JCTree resource : tree.resources) {
                JCList<Type> closeableSupertypes = resource.type.isCompound() ?
                    types.interfaces(resource.type).prepend(types.supertype(resource.type)) :
                    JCList.of(resource.type);
            ...
            scan(tree.body);
            JCList<Type> thrownInTry = allowImprovedCatchAnalysis ?
                chk.union(thrown, JCList.of(syms.runtimeExceptionType, syms.errorType)) :
                thrown;
            thrown = thrownPrev;
            caught = caughtPrev;

            JCList<Type> caughtInTry = JCList.nil();
            for (JCList<JCCatch> l = tree.catchers; l.nonEmpty(); l = l.tail) {
                JCVariableDecl param = l.head.param;
                JCList<JCExpression> subClauses = TreeInfo.isMultiCatch(l.head) ?
                        ((JCTypeUnion)l.head.param.vartype).alternatives :
                        JCList.of(l.head.param.vartype);
                JCList<Type> ctypes = JCList.nil();
                JCList<Type> rethrownTypes = chk.diff(thrownInTry, caughtInTry);
                for (JCExpression ct : subClauses) {
                    Type exc = ct.type;
            ...
            if (tree.finalizer != null) {
                JCList<Type> savedThrown = thrown;
                thrown = JCList.nil();
        ...
        void checkCaughtType(DiagnosticPosition pos, Type exc, JCList<Type> thrownInTry, JCList<Type> caughtInTry) {
            if (chk.subset(exc, caughtInTry)) {
                ...
                JCList<Type> catchableThrownTypes = chk.intersect(JCList.of(exc), thrownInTry);
        ...
        public void visitApply(JCMethodInvocation tree) {
            scan(tree.meth);
            scan(tree.args);
            for (JCList<Type> l = tree.meth.type.getThrownTypes(); l.nonEmpty(); l = l.tail)
        ...
        public void visitNewClass(JCNewClass tree) {
            ...
            scan(tree.args);
           // scan(tree.def);
            for (JCList<Type> l = tree.constructorType.getThrownTypes();
                 l.nonEmpty();
                 l = l.tail) {
                markThrown(tree, l.head);
            }
            ...
            try {
                ...
                if (tree.def != null)
                    for (JCList<Type> l = tree.constructor.type.getThrownTypes();
                         l.nonEmpty();
                         l = l.tail) {
        ...
        public void visitLambda(JCLambda tree) {
            ...
            JCList<Type> prevCaught = caught;
            JCList<Type> prevThrown = thrown;
    ...
    class LambdaFlowAnalyzer extends FlowAnalyzer {
        JCList<Type> inferredThrownTypes;
        boolean inLambda;
        @Override
        public void visitLambda(JCLambda tree) {
            if ((tree.type != null &&
            ...
            JCList<Type> prevCaught = caught;
            JCList<Type> prevThrown = thrown;
            ListBuffer<Flow.FlowAnalyzer.FlowPendingExit> prevPending = pendingExits;
            inLambda = true;
            try {
                pendingExits = new ListBuffer<>();
                caught = JCList.of(syms.throwableType);
                thrown = JCList.nil();
                scan(tree.body);
     ...
     *************************************************************************/
         ...
         */
        void scanExprs(JCList<? extends JCExpression> trees) {
            if (trees != null)
                for (JCList<? extends JCExpression> l = trees; l.nonEmpty(); l = l.tail)
                    scanExpr(l.head);
        }
        ...
        public void visitMethodDef(JCMethodDecl tree) {
            ...
            try {
                ...
                try {
                    ...
                    JCList<AssignPendingExit> exits = pendingExits.toList();
                    pendingExits = new ListBuffer<>();
                    while (exits.nonEmpty()) {
        ...
        // where
            ...
            private void addVars(JCList<JCStatement> stats, final Bits inits,
                                 final Bits uninits) {
     ...
     *************************************************************************/
        ...
        public void analyzeTree(Env<com.flint.tools.flintc.comp.AttrContext> env, JCTree tree, TreeMaker make) {
            try {
                attrEnv = env;
                Flow.this.make = make;
                pendingExits = new ListBuffer<>();
