...
public class ListBuffer<A> extends AbstractQueue<A> {
    ...
    private void copy() {
        if (elems.nonEmpty()) {
            ...
            elems = last = JCList.of(orig.head);

            while ((orig = orig.tail).nonEmpty()) {
                last.tail = JCList.of(orig.head);
                last = last.tail;
            }
        }
    }
     ...
     */
    public ListBuffer<A> appendList(JCList<A> xs) {
        while (xs.nonEmpty()) {
            append(xs.head);
            xs = xs.tail;
        }
    ...
    public Iterator<A> iterator() {
        return new Iterator<A>() {
            JCList<A> elems = ListBuffer.this.elems;
