...
public class GraphUtils {
     ...
     */
    public static <D, N extends TarjanNode<D, N>> JCList<? extends JCList<? extends N>> tarjan(Iterable<? extends N> nodes) {
    ...
    private static class Tarjan<D, N extends TarjanNode<D, N>> {
        ...
        com.flint.tools.flintc.util.ListBuffer<JCList<N>> sccs = new com.flint.tools.flintc.util.ListBuffer<>();

        /** Stack of all reacheable nodes from given root. */
        com.flint.tools.flintc.util.ListBuffer<N> stack = new com.flint.tools.flintc.util.ListBuffer<>();

        private JCList<? extends JCList<? extends N>> findSCC(Iterable<? extends N> nodes) {
            for (N node : nodes) {
                if (node.index == -1) {
                    findSCC(node);
                }
        ...
        private void addSCC(N v) {
            N n;
            com.flint.tools.flintc.util.ListBuffer<N> cycle = new ListBuffer<>();
            do {
                n = stack.remove();
                n.active = false;
                cycle.add(n);
