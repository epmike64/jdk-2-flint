...
public class Pretty extends JCTree.Visitor {
     ...
     */
    public <T extends JCTree> void printExprs(JCList<T> trees, String sep) throws IOException {
        if (trees.nonEmpty()) {
            printExpr(trees.head);
            for (JCList<T> l = trees.tail; l.nonEmpty(); l = l.tail) {
                print(sep);
                printExpr(l.head);
            }
        }
     ...
     */
    public <T extends JCTree> void printExprs(JCList<T> trees) throws IOException {
        printExprs(trees, ", ");
    }

    /** Derived visitor method: print list of statements, each on a separate line.
     */
    public void printStats(JCList<? extends JCTree> trees) throws IOException {
    ...
    public void printAnnotations(JCList<JCAnnotation> trees) throws IOException {
        for (JCList<JCAnnotation> l = trees; l.nonEmpty(); l = l.tail) {
            printStat(l.head);
            println();
            align();
        }
    ...
    public void printTypeAnnotations(JCList<JCAnnotation> trees) throws IOException {
        for (JCList<JCAnnotation> l = trees; l.nonEmpty(); l = l.tail) {
            printExpr(l.head);
            print(" ");
        }
...
//where
     ...
     *  {@literal "<...>"} brackets.
     */
    public void printTypeParameters(JCList<JCTypeParameter> trees) throws IOException {
        if (trees.nonEmpty()) {
            print("<");
            printExprs(trees);
            print(">");
    ...
    /** Print a block.
     */
    public void printBlock(JCList<? extends JCTree> stats) throws IOException {
        print("{");
        println();
        indent();
        printStats(stats);
    ...
    /** Print a block.
     */
    public void printEnumBody(JCList<JCTree> stats) throws IOException {
        print("{");
        println();
        indent();
        boolean first = true;
        for (JCList<JCTree> l = stats; l.nonEmpty(); l = l.tail) {
        ...
        try {
            ...
            if (tree.init.nonEmpty()) {
                if (tree.init.head.hasTag(VARDEF)) {
                    ...
                    for (JCList<JCStatement> l = tree.init.tail; l.nonEmpty(); l = l.tail) {
    ...
    public void visitNewArray(JCNewArray tree) {
        try {
            if (tree.elemtype != null) {
                ...
                int i = 0;
                JCList<JCList<JCAnnotation>> da = tree.dimAnnotations;
    ...
    public String operatorName(JCTree.JCTreeTag tag) {
        switch(tag) {
            case POS:     return "+";
            case NEG:     return "-";
            case NOT:     return "!";
