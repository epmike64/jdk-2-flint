...
public class TreeCopier<P> implements TreeVisitor<JCTree,P> {
    ...
    public <T extends JCTree> JCList<T> copy(JCList<T> trees) {
        return copy(trees, null);
    }

    public <T extends JCTree> JCList<T> copy(JCList<T> trees, P p) {
        if (trees == null)
            return null;
    ...
    public JCTree visitAnnotatedType(AnnotatedTypeTree node, P p) {
        ...
        JCList<JCAnnotation> annotations = copy(t.annotations, p);
    ...
    public JCTree visitAnnotation(AnnotationTree node, P p) {
        ...
        if (t.getKind() == Kind.TYPE_ANNOTATION) {
    ...
    public JCTree visitBlock(BlockTree node, P p) {
        ...
        JCList<JCStatement> stats = copy(t.stats, p);
    ...
    public JCTree visitClass(ClassTree node, P p) {
        ...
        JCList<JCTypeParameter> typarams = copy(t.typarams, p);
        JCExpression extending = copy(t.extending, p);
        JCList<JCExpression> implementing = copy(t.implementing, p);
        JCList<JCTree> defs = copy(t.defs, p);
    ...
    public JCTree visitErroneous(ErroneousTree node, P p) {
        ...
        JCList<? extends JCTree> errs = copy(t.errs, p);
    ...
    public JCTree visitForLoop(ForLoopTree node, P p) {
        ...
        JCList<JCStatement> init = copy(t.init, p);
        JCExpression cond = copy(t.cond, p);
        JCList<JCExpressionStatement> step = copy(t.step, p);
        JCStatement body = copy(t.body, p);
    ...
    public JCTree visitMethod(MethodTree node, P p) {
        ...
        JCList<JCTypeParameter> typarams = copy(t.typarams, p);
        JCList<JCVariableDecl> params = copy(t.params, p);
        JCVariableDecl recvparam = copy(t.recvparam, p);
        JCList<JCExpression> thrown = copy(t.thrown, p);
        JCBlock body = copy(t.body, p);
    ...
    public JCTree visitNewArray(NewArrayTree node, P p) {
        ...
        JCList<JCExpression> dims = copy(t.dims, p);
        JCList<JCExpression> elems = copy(t.elems, p);
    ...
    public JCTree visitSwitch(SwitchTree node, P p) {
        ...
        JCList<JCCase> cases = copy(t.cases, p);
    ...
    public JCTree visitTry(TryTree node, P p) {
        JCTry t = (JCTry) node;
        JCList<JCTree> resources = copy(t.resources, p);
        JCBlock body = copy(t.body, p);
        JCList<JCCatch> catchers = copy(t.catchers, p);
    ...
    public JCTree visitParameterizedType(ParameterizedTypeTree node, P p) {
        ...
        JCExpression clazz = copy(t.clazz, p);
        JCList<JCExpression> arguments = copy(t.arguments, p);
    ...
    public JCTree visitUnionType(UnionTypeTree node, P p) {
        ...
        JCList<JCExpression> components = copy(t.alternatives, p);
    ...
    public JCTree visitIntersectionType(IntersectionTypeTree node, P p) {
        ...
        JCList<JCExpression> bounds = copy(t.bounds, p);
    ...
    public JCTree visitTypeParameter(TypeParameterTree node, P p) {
        ...
        JCList<JCAnnotation> annos = copy(t.annotations, p);
        JCList<JCExpression> bounds = copy(t.bounds, p);
    ...
    public JCTree visitModule(ModuleTree node, P p) {
        ...
        JCList<JCDirective> directives = copy(t.directives);
    ...
    public JCExports visitExports(ExportsTree node, P p) {
        ...
        JCList<JCExpression> moduleNames = copy(t.moduleNames, p);
    ...
    public JCProvides visitProvides(ProvidesTree node, P p) {
        ...
        JCList<JCExpression> implNames = copy(t.implNames, p);
    ...
    public JCTree visitOther(Tree node, P p) {
        ...
        switch (tree.getTag()) {
            case LETEXPR: {
                ...
                JCList<JCVariableDecl> defs = copy(t.defs, p);
                JCExpression expr = copy(t.expr, p);
